...
Logistic Regression(LR) is a widely used statistic model for classification problems. FATE provided two kinds of federated LR that are Homogeneous LR (HomoLR) and Heterogeneous LR (HeteroLR). 
...
On the other hands, in Hetero mode, all parties involved should have enough amount of overlapping samples (i.e., samples with the same ids). The combination of sub-models from all parties form a whole federated model. 
...
<div style="text-align:center", align=center>
<img src="./images/HeteroLR-NN.png" alt="architecture" width="500" height="300" />
<br/>
Figure 3: Federated HeteroLR with Neural Network Principle </div>

As shown in Figure 3, neural networks are added between the raw input data and the LR model serving as feature extractors that extract representative features from raw input data of various types. Neural networks can be CNN for processing images, RNN for processing text, autoencoder for processing general numerical vectors and many others. Currently we only used autoencoder in this algorithm. We will add other models in the near future.
