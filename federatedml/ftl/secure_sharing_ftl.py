...
class SecureSharingFTLGuestModel(SecureSharingParty):
    ...
    def set_batch(self, X, y, overlap_indexes=None, non_overlap_indexes=None):
        self.X = X
        self.y = y
    ...
    def _compute_components(self):
        ...
        self.ave_y_non_overlap = self.y[self.non_overlap_indexes] / len(self.y)
    ...
    def compute_share_for_guest_non_overlap_federated_layer_grad(self, alpha_t, beta_t):
        op_id = "mul_op_5"
        alpha_s, beta_s = self.alpha_beta_computer.get_shares_for_mul_op_alpha_beta(op_id)
        self.guest_non_overlap_federated_layer_grad_g = compute_multiply_share(alpha_s, alpha_t, beta_s, beta_t,
                                                                               self.alpha_beta_computer.get_shares_for_mul_op_beaver_triple(
                                                                                   op_id))
    ...
    def compute_share_for_guest_overlap_federated_layer_grad(self):
        self.guest_overlap_federated_layer_grad_g = compute_add_share(self.guest_part_overlap_federated_layer_grad_g,
    ...
    def compute_shares_for_alpha_beta_for_guest_grad_W(self, global_index, op_id=None):
        ...
        self.guest_federated_layer_grad_g[self.non_overlap_indexes, :] = self.guest_non_overlap_federated_layer_grad_g
    ...
    def compute_share_for_guest_grad_W(self, alpha_t, beta_t):
        op_id = "mul_op_7"
        alpha_s, beta_s = self.alpha_beta_computer.get_shares_for_mul_op_alpha_beta(op_id)
        self.guest_grad_W_g = compute_sum_of_multiply_share(alpha_s, alpha_t, beta_s, beta_t,
                                                            self.alpha_beta_computer.get_shares_for_mul_op_beaver_triple(
                                                                op_id), axis=0)
        return self.guest_grad_W_g
    ...
    def compute_share_for_guest_grad_b(self, alpha_t, beta_t):
        op_id = "mul_op_8"
        alpha_s, beta_s = self.alpha_beta_computer.get_shares_for_mul_op_alpha_beta(op_id)
        self.guest_grad_b_g = compute_sum_of_multiply_share(alpha_s, alpha_t, beta_s, beta_t,
                                                            self.alpha_beta_computer.get_shares_for_mul_op_beaver_triple(
                                                                op_id), axis=0)
        return self.guest_grad_b_g

    def receive_gradients(self, gradients):
        # receive grad_W_g and grad_b_g and reconstruct grad_W and grad_b
        grad_W = gradients[0] + self.guest_grad_W_g
        grad_b = gradients[1] + self.guest_grad_b_g
    ...
    def compute_share_for_loss(self):
        # print("self.overlap_y_phi_uB", self.overlap_y_phi_uB_g.shape)
        # print("self.phi_overlap_uB_2_phi", self.phi_overlap_uB_2_phi_g.shape)
        # print("self.uA_uB", self.uA_uB_g.shape)
        loss_g = (np.sum(self.overlap_y_phi_uB_g) + 1.0 / 8 * np.sum(self.phi_overlap_uB_2_phi_g)) + np.sum(
            self.uA_uB_g) + len(self.y_overlap) * np.log(2)
        return loss_g
    ...
    def compute_share_for_host_overlap_federated_layer_grad(self):
        self.host_overlap_federated_layer_grad_g = compute_add_share(self.overlap_uB_y_2_phi_2_g,
...
class SecureSharingFTLHostModel(SecureSharingParty):
    ...
    def set_batch(self, X, overlap_indexes=None, non_overlap_indexes=None):
        self.X = X
        self.overlap_indexes = overlap_indexes
        self.non_overlap_indexes = non_overlap_indexes
    ...
    def compute_share_for_host_overlap_federated_layer_grad(self):
        self.host_overlap_federated_layer_grad_h = compute_add_share(self.overlap_uB_y_2_phi_2_h,
    ...
    def compute_share_for_guest_non_overlap_federated_layer_grad(self, alpha_t, beta_t):
        op_id = "mul_op_5"
        alpha_s, beta_s = self.alpha_beta_computer.get_shares_for_mul_op_alpha_beta(op_id)
        self.guest_non_overlap_federated_layer_grad_h = compute_multiply_share(alpha_s, alpha_t, beta_s, beta_t,
                                                                               self.alpha_beta_computer.get_shares_for_mul_op_beaver_triple(
                                                                                   op_id))
    ...
    def compute_share_for_guest_overlap_federated_layer_grad(self):
        self.guest_overlap_federated_layer_grad_h = compute_add_share(self.guest_part_overlap_federated_layer_grad_h,
    ...
    def compute_shares_for_alpha_beta_for_guest_grad_W(self, guest_overlap_grads_W_h, global_index, op_id=None):
        ...
        self.guest_federated_layer_grad_h[self.non_overlap_indexes, :] = self.guest_non_overlap_federated_layer_grad_h
    ...
    def compute_share_for_loss(self):
        # print("self.overlap_y_phi_uB", self.overlap_y_phi_uB_h.shape)
        # print("self.phi_overlap_uB_2_phi", self.phi_overlap_uB_2_phi_h.shape)
        # print("self.uA_uB", self.uA_uB_h.shape)
        # print("len(self.ave_y_overlap_h) * np.log(2)", len(self.ave_y_overlap_h) * np.log(2))
        loss_h = np.sum(self.overlap_y_phi_uB_h) + 1.0 / 8 * np.sum(self.phi_overlap_uB_2_phi_h) + np.sum(self.uA_uB_h)
        return loss_h
...
class LocalSecureSharingFederatedTransferLearning(object):
    ...
    def fit(self, X_A, X_B, y, overlap_indexes, non_overlap_indexes, global_index):
        ...
        host_grad_W = host_grad_W_h + host_grad_W_g
        host_grad_b = host_grad_b_h + host_grad_b_g

        self.host.receive_gradients([host_grad_W, host_grad_b])
        ...
        loss_h = self.host.compute_share_for_loss()
        loss_g = self.guest.compute_share_for_loss()
        loss = loss_g + loss_h
