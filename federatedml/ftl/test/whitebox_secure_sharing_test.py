...
class TestPlainGradients(unittest.TestCase):
    ...
    def test_party_a_gradient_checking_test(self):
        ...
        for global_index in range(len(party_a_bt_map)):
            ...
            host.set_batch(U_B, overlap_indexes, non_overlap_indexes)
            partyA.set_batch(U_A, y, non_overlap_indexes, overlap_indexes)
            partyB.set_batch(U_B, overlap_indexes)
            ...
            guest_federated_layer_grad[non_overlap_indexes, :] = guest_non_overlap_federated_layer_grad
            ...
            loss_h = host.compute_share_for_loss()
            loss_g = guest.compute_share_for_loss()
            ...
            # overlap_y_phi_uB = guest.overlap_y_phi_uB_g + host.overlap_y_phi_uB_h
            # phi_overlap_uB_2_phi = guest.phi_overlap_uB_2_phi_g + host.phi_overlap_uB_2_phi_h
            # uA_uB = guest.uA_uB_g + host.uA_uB_h
            # print("overlap_y_phi_uB_g", np.sum(overlap_y_phi_uB))
            # print("phi_overlap_uB_2_phi", np.sum(phi_overlap_uB_2_phi))
            # print("uA_uB", np.sum(uA_uB))
