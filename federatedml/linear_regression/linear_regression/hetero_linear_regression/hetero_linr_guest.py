...
class HeteroLinRGuest(HeteroLinRBase):
    ...
    def compute_intermediate(self, data_instances, coef_, intercept_, batch_index=-1):
        wx = self.compute_wx(data_instances, coef_, intercept_)
        self.guest_wxy = wx.join(data_instances, lambda wx, d: wx - d.label)
        guest_loss = self.gradient_operator.compute_loss(data_instances, coef_, intercept_)
    ...
    def fit(self, data_instances):
        ...
        while self.n_iter_ < self.max_iter:
            ...
            for batch_data_index in batch_data_generator:
                ...
                self.compute_intermediate(batch_feat_inst, self.coef_, self.intercept_, batch_index)
                ...
                residual = self.gradient_operator.compute_residual(batch_feat_inst, self.coef_, self.intercept_, host_forward_wx)
