    ...
    def fit(self, data_instances):
        ...
        while self.n_iter_ < self.max_iter:
            ...
            while batch_index < self.batch_num:
                ...
                loss = self.gradient_operator.compute_loss(batch_feat_inst, self.wx, consts.HOST)

                if self.updater is not None:
                    loss_regular = self.updater.loss_norm(self.coef_)
                    #LOGGER.info(str(self.coef_))
            ...
            LOGGER.info("Iteration time: {}".format(time.time() - start))
