...
class Evaluation(ModelBase):
    ...
    @staticmethod
    def confusion_matrix(sorted_labels: list, sorted_pred_scores: list, score_thresholds: list, ret: list):
    ...
    def neg_pos_count(self, labels):
        pos_num = ((labels == self.pos_label) + 0).sum()
        neg_num = len(labels) - pos_num
        return pos_num, neg_num

    @staticmethod
    def sort_score_and_label(labels, pred_scores):
    ...
    def ks_fast(self, labels, pred_scores):

        sorted_labels, sorted_scores = self.sort_score_and_label(labels, pred_scores)
        ...
        confusion_mat = self.confusion_matrix(sorted_labels, sorted_scores, score_threshold, ret=['tp', 'fp'])

        pos_num, neg_num = self.neg_pos_count(sorted_labels)
    ...
    def lift_fast(self, labels, pred_scores):
        ...
        confusion_mat = self.confusion_matrix(sorted_labels, sorted_scores, score_threshold,
                                              ret=['tp', 'fp', 'fn', 'tn'])

        labels_nums = np.zeros(len(confusion_mat['tp'])) + len(labels)
        ...
        return lifts_y, lifts_x, thresholds

    def _gain_helper(self):
        pass

    def gain_fast(self, tpr, fpr, pos_num, neg_num):
        pass
