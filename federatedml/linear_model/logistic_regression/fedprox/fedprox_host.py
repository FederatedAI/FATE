...
class fedproxHost(FedProxBase):
    def __init__(self):
        super(fedproxHost, self).__init__()
    ...
    def fit(self, data_instances, validate_data=None):
        ...
        Args: 
            data_instances (pd.DataFrame): the actual data provided by host
        ...
        LOGGER.debug("**FedProx** Start data count: {}".format(data_instances.count()))

        #Check if the data is present and its schema and initialize validation to check model performance
        self._abnormal_detection(data_instances)
        self.init_schema(data_instances)
        validation_strategy = self.init_validation_strategy(data_instances, validate_data)

        #Set public key and encrypt the initialized model weights 
        ...
        LOGGER.debug("**FedProx** After init, model_weights: {}".format(self.model_weights.unboxed))
        ...
        #Send encrypted batch size and batch number to arbiter to track the model training progress
        if self.use_encrypt:
            re_encrypt_times = (total_batch_num - 1) // self.re_encrypt_batches + 1
            LOGGER.debug("**FedProx** re_encrypt_times is :{}, batch_size: {}, total_batch_num: {}, re_encrypt_batches: {}".format(
        ...
        LOGGER.debug("*FedProx** Current data count: {}".format(total_data_num))
        self.transfer_variable.host_size.remote(total_data_num, role = consts.ARBITER, idx = -1) # send host size to arbiter
        LOGGER.debug("**FedProx** host size sent")
        ...
        while self.n_iter_ < self.max_iter:
            ...
            if self.n_iter_ > 0 and self.n_iter_ % self.aggregate_iters == 0:
                ...
                if not self.use_encrypt:
                    ...
                    LOGGER.info("**FedProx** n_iters: {}, loss: {}".format(self.n_iter_, loss))
                degree = 0
                self.is_converged = self.aggregator.get_converge_status(suffix=(self.n_iter_,))
                LOGGER.info("**FedProx** n_iters: {}, is_converge: {}".format(self.n_iter_, self.is_converged))
            ...
            for batch_data in batch_data_generator:
                n = batch_data.count()
                degree += n
                LOGGER.debug('**FedProx** before compute_gradient')
                ...
                if self.use_encrypt and batch_num % self.re_encrypt_batches == 0:
                    LOGGER.debug("**FedProx** Before accept re_encrypted_model, batch_iter_num: {}".format(batch_num))
        ...
        LOGGER.info("**FedProx** Finish Training task, total iters: {}".format(self.n_iter_))
