...
class Guest(hetero_lr_gradient_sync.Guest):
    ...
    def compute_lr_gradient(self, data_instances, lr_variables,
        ...
        guest_forward = encrypted_calculator[batch_index].encrypt(wx)
        host_forward = self.get_host_forward(suffix=current_suffix)
        aggregate_forward_res = guest_forward.join(host_forward, lambda g, h: g + h)
