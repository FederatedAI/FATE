...
class HeteroFeatureBinningGuest(BaseHeteroFeatureBinning):
    ...
    def fit(self, data_instances):
        ...
        LOGGER.debug("[federation] Send encrypted_label time: {}".format(end_trans_time - trans_time))
        ...
        encrypted_bin_sum = federation.get(name=self.transfer_variable.encrypted_bin_sum.name,
                                           ...
                                           idx=0)
        end_trans_time = time.time()
        LOGGER.debug("[federation] Get encrypted_bin_sum time: {}".format(end_trans_time - trans_time))
        self.communication_time += (end_trans_time - trans_time)
