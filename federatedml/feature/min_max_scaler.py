...
class MinMaxScaler(object):
        ...
        if min_value is None and max_value is not None:
            ...
            if isinstance(max_value, Iterable):
                if len(list(max_value)) != len(min_value_list):
                    raise ValueError("Size of feat_upper is not equal to column of data, {} != {}".format(len(list(max_value)), len(min_value_list)))
            ...
            if isinstance(min_value, Iterable):
                if len(list(min_value)) != len(max_value_list):
                    raise ValueError("Size of feat_lower is not equal to column of data, {} != {}".format(len(list(max_value)), len(max_value_list)))
            ...
            if check_value is not None:
                if self.area == 'all':
                    if not isinstance(check_value, int) and not isinstance(check_value, float):
                        raise ValueError("for area is all, {} should be int or float, not {}".format(check_value, type(check_value)))

                elif self.area == 'col':
                    if not isinstance(check_value, Iterable):
                        raise ValueError("for area is col, {} should be Iterable, not {}".format(check_value, type(check_value)))
        ...
        if self.feat_upper is not None and self.feat_lower is not None:
            if self.area == 'all':
                if float(self.feat_upper) < float(self.feat_lower):
                    raise ValueError("for area is all, feat_upper should not less than feat_lower, but {} < {}".format(self.feat_upper, self.feat_lower))
            elif self.area == 'col':
                if len(list(self.feat_upper)) != len(list(self.feat_lower)):
                    raise ValueError("for area is col, sizeof feat_upper should equal to the sizeof feat_lower, but {} != {}".format(len(list(self.feat_upper)), len(list(self.feat_lower))))


                for i in range(len(list(self.feat_upper))):
                    if float(self.feat_upper[i]) < float(self.feat_lower[i]):
                        raise ValueError("for area is col, feat_upper[{}] should not less than feat_lower[{}], but {} < {}".format(i, i, self.feat_upper[i], self.feat_lower[i]))
        
        if self.out_upper is not None and self.out_lower is not None:
            if float(self.out_upper) < float(self.out_lower):
                raise ValueError("for area is all, out_upper should not less than out_lower, but {} < {}".format(self.out_upper, self.out_lower))
    ...
    def fit(self, data):
        ...
        f = functools.partial(MinMaxScaler.__scale_with_cols_for_instance, max_value_list=max_value, min_value_list=min_value, scale_value_list=data_scale, out_lower=out_lower, out_scale=out_scale)
    ...
    def transform(self, data, cols_transform_value):
        if len(cols_transform_value) == 1:
            if len(cols_transform_value[0]) != 4:
                raise ValueError("if cols_transform_value size is 1, it should has 4 elements, but {}".format(len(cols_transform_value[0])))
            ...
            f = functools.partial(MinMaxScaler.__scale_with_value_for_instance, max_value=max_value, min_value=min_value, scale_value=data_scale, out_lower=out_lower, out_scale=out_scale)
        else:
            max_value = []
            min_value = []
            out_upper = []
            ...
            f = functools.partial(MinMaxScaler.__scale_with_cols_for_instance, max_value_list=max_value, min_value_list=min_value, scale_value_list=data_scale, out_lower=out_lower[0], out_scale=out_scale)
