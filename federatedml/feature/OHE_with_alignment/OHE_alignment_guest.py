...
# ptvsd.enable_attach(address =('0.0.0.0',6758))
...
class OHEAlignmentGuest(OHEAlignmentBase):
    ...
    def fit(self, data_instances):
        ...
        LOGGER.debug("**AIFEL** new col_maps is: {}".format(col_maps))

        #Feature alignment is executed in a 'switch' manner
        if self.need_alignment:

            #Send col_maps to arbiter
            self.transfer_variable.guest_columns.remote("{}".format(col_maps), role = consts.ARBITER, idx = -1) # send guest size to arbiter
            LOGGER.debug("**AIFEL** guest column sent")
