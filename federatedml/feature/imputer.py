...
class ImputerProcess(object):
    ...
    @staticmethod
    def __replace_missing_value_with_cols_transform_value_format(data, transform_list, missing_value_list, output_format):
    ...
    def __get_cols_transform_min_value(self, data):
        min_lists = data.mapPartitions(ImputerProcess.__get_min)
    ...
    def __get_cols_transform_max_value(self, data):
        max_lists = data.mapPartitions(ImputerProcess.__get_max)
    ...
    def __get_cols_transform_mean_value(self, data):
        get_mean_results = data.mapPartitions(ImputerProcess.__get_mean)
        cols_sum = None
        cols_counter = None
        ...
        for value_tuple in list(get_mean_results.collect()):
            ...
            for i in range(len(value_sum)):
                if value_sum[i] is None:
                    print("col {} of cols_sum is None, continue".format(i))
                    continue
            ...
            for i in range(len(value_counter)):
                if value_counter[i] is None:
                    print("col {} of cols_counter is None, continue".format(i))
                    continue
    ...
    def __replace(self, data, replace_method, replace_value=None, output_format=None):
        ...
        if replace_method is not None and replace_method != consts.DESIGNATED:
            ...
            if output_format is not None:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_cols_transform_value_format, transform_list=cols_transform_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_cols_transform_value, transform_list=cols_transform_value, missing_value_list=self.imputer_value_list)

            transform_data = data.mapValues(f)
            self.print_table(transform_data)
            print("finish replace missing value with cols transform value, replace method is {}".format(replace_method))
            return transform_data, cols_transform_value
        else:
            if replace_value is None:
                raise ValueError("Replace value should not be None")
            if output_format is not None:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_replace_value_format, replace_value=replace_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_replace_value, replace_value=replace_value, missing_value_list=self.imputer_value_list)
            transform_data = data.mapValues(f)
            self.print_table(transform_data)

            print("finish replace missing value with replace value {}".format(replace_value))
            return transform_data, replace_value
            
    def __transform_replace(self, data, transform_value, replace_area,  output_format):
        LOGGER.debug("enter transform, data is {}, transform_value is {}, replace_area is {}, output_format is {}".format(data, transform_value, replace_area, output_format))
        if replace_area == 'all':
            if output_format is not None:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_replace_value_format, replace_value=transform_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_replace_value, replace_value=transform_value, missing_value_list=self.imputer_value_list)
        elif replace_area == 'col':
            if output_format is not None:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_cols_transform_value_format, transform_list=transform_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(ImputerProcess.__replace_missing_value_with_cols_transform_value, replace_value=transform_value, missing_value_list=self.imputer_value_list)
        else:
            raise ValueError("Unknown replace area {} in ImputerProcess".format(replace_area))
        LOGGER.debug("begin to transform") 
        transform_data = data.mapValues(f)
        LOGGER.debug("finish to transform") 
        self.print_table(transform_data)
    ...
    def fit(self, data, replace_method=None, replace_value=None, output_format = consts.ORIGIN):
        ...
        if isinstance(replace_method, str):
            ...
            if replace_method not in self.support_replace_method:
                raise ValueError("Unknown replace method in ImputerProcess")
    ...
    def transform(self, data, replace_method=None, transform_value=None, output_format = consts.ORIGIN):
        LOGGER.debug("enter here, data is {}".format(data))
        LOGGER.debug("enter here, replace_method is {}".format(replace_method))
        if output_format not in self.support_output_format:
            raise ValueError("Unsupport output_format:{}".format(output_format))
        ...
        if isinstance(replace_method, str):
            replace_method = replace_method.lower()
            if replace_method not in self.support_replace_method:
                raise ValueError("Unknown replace method {} in ImputerProcess".format(replace_method))
            
            if replace_method not in self.support_replace_area:
                raise ValueError("Unknown replace area of method {} in ImputerProcess".format(replace_method))
