...
class Imputer(object):
    ...
    def __replace(self, data, replace_method, replace_value=None, output_format=None):
        if replace_method is not None and replace_method != consts.DESIGNATED:
            ...
            if output_format is not None:
                f = functools.partial(Imputer.__replace_missing_value_with_cols_transform_value_format, transform_list=cols_transform_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(Imputer.__replace_missing_value_with_cols_transform_value, transform_list=cols_transform_value, missing_value_list=self.imputer_value_list)

            transform_data = data.mapValues(f)
            LOGGER.debug("finish replace missing value with cols transform value, replace method is {}".format(replace_method))
            return transform_data, cols_transform_value
        else:
            if replace_value is None:
                raise ValueError("Replace value should not be None")
            if output_format is not None:
                f = functools.partial(Imputer.__replace_missing_value_with_replace_value_format, replace_value=replace_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(Imputer.__replace_missing_value_with_replace_value, replace_value=replace_value, missing_value_list=self.imputer_value_list)
            transform_data = data.mapValues(f)
    ...
    def __transform_replace(self, data, transform_value, replace_area,  output_format):
        if replace_area == 'all':
            if output_format is not None:
                f = functools.partial(Imputer.__replace_missing_value_with_replace_value_format, replace_value=transform_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(Imputer.__replace_missing_value_with_replace_value, replace_value=transform_value, missing_value_list=self.imputer_value_list)
        elif replace_area == 'col':
            if output_format is not None:
                f = functools.partial(Imputer.__replace_missing_value_with_cols_transform_value_format, transform_list=transform_value, missing_value_list=self.imputer_value_list, output_format=output_format)
            else:
                f = functools.partial(Imputer.__replace_missing_value_with_cols_transform_value, replace_value=transform_value, missing_value_list=self.imputer_value_list)
        else:
