...
class SecureSumAggregatorTransVar(BaseTransferVariables):
    def __init__(self, flowid=0):
        super().__init__(flowid)
        self.AggregatorTransVar.ModelScatterTransVar.client_model = self._create_variable(name='AggregatorTransVar.ModelScatterTransVar.client_model', src=['guest', 'host'], dst=['arbiter'])
        self.AggregatorTransVar.ModelBroadcasterTransVar.server_model = self._create_variable(name='AggregatorTransVar.ModelBroadcasterTransVar.server_model', src=['arbiter'], dst=['guest', 'host'])
