...
```
#Get code
wget https://webank-ai-1251170195.cos.ap-guangzhou.myqcloud.com/docker_standalone-fate-1.4.6.tar.gz
tar -xzvf docker_standalone-fate-1.4.6.tar.gz

#Execute the command
cd docker_standalone-fate-1.4.6
...
2. Download the compressed package of stand-alone version and decompress it.
   ...
   ```
   wget https://webank-ai-1251170195.cos.ap-guangzhou.myqcloud.com/standalone-fate-master-1.4.6.tar.gz
   tar -xzvf  standalone-fate-master-1.4.6.tar.gz
   ```

3. Enter FATE directory and execute the init.sh.

   ```
   cd standalone-fate-master-1.4.6
   source init.sh init
   ```
