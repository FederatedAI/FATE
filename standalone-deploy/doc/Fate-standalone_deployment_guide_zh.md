...
3. 执行之前，请检查8080是否已被占用。 如果要再次执行，请使用docker命令删除以前的容器和镜像。
   ...
   ```
   #获取安装包
   wget https://webank-ai-1251170195.cos.ap-guangzhou.myqcloud.com/docker_standalone_fate_1.5.2.tar.gz
   tar -xzvf docker_standalone_fate_1.5.2.tar.gz
   
   #执行部署
   cd docker_standalone_fate_1.5.2
...
2. 下载独立版本的压缩包并解压缩。
   ...
   ```
   wget https://webank-ai-1251170195.cos.ap-guangzhou.myqcloud.com/standalone_fate_master_1.5.2.tar.gz
   tar -xzvf  standalone_fate_master_1.5.2.tar.gz
   ```

3. 进入FATE目录并执行init.sh。

   ```
   cd standalone_fate_master_1.5.2
   sh init.sh init
   ```
