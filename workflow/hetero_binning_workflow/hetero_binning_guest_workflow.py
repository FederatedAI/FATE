...
class HeteroBinningGuestWorkflow(WorkFlow):
    ...
    def save_binning_result(self, iv_attrs):
        save_dict = {}
    ...
    def run(self):
        ...
        if self.workflow_param.method == "binning":
            ...
            if self.binning_param.local_only:
                iv_attrs = self.model.fit_local(train_data_instance)
            else:
                iv_attrs = self.model.fit(train_data_instance)
            self.save_binning_result(iv_attrs)
        else:
