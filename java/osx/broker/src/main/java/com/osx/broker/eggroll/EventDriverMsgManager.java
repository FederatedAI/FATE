...
public class EventDriverMsgManager implements Lifecycle {
    ...
    public void init() {
        ...
        MsgEventCallback  eggrollCreateUserCallback =  new MsgEventCallback(){
            @Override
            public synchronized  void callback(TransferQueue queue , Message message) {
                String topic = queue.getTransferId();
                if(consumerManager.getEventDrivenConsumer(topic)==null){
                    consumerManager.createEventDrivenConsumer(topic,new PushEventHandler());
                };
            }
        };
       // Dict.EGGROLL_SEND_TOPIC_PREFIX
        ServiceContainer.transferQueueManager.addMsgCallBackRule((queue -> {
            if(queue.getTransferId().startsWith(Dict.EGGROLL_SEND_TOPIC_PREFIX)){
                return true;
            }
            return false;
        }), Lists.newArrayList(eggrollCreateUserCallback,dispatchCallback));

        ServiceContainer.transferQueueManager.addMsgCallBackRule((queue->{
            if(queue.getTransferId().startsWith(Dict.EGGROLL_BACK_TOPIC_PREFIX)){
                return true;
            }
            return false;
