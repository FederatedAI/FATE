...
public class TransferQueueManager {
    ...
    public  Pcp.Outbound  applyFromMaster(Context context , Pcp.Inbound  inbound){
        if(!isMaster()){
            ManagedChannel managedChannel = GrpcConnectionFactory.createManagedChannel(this.getMasterAddress());
            ...
            String topic = inbound.getMetadataMap().get(Pcp.Metadata.MessageTopic);
            String instanceId = inbound.getMetadataMap().get(Pcp.Metadata.InstanceId);
            String sessionId = inbound.getMetadataMap().get(Pcp.Header.SessionID);
            ...
            outboundBuilder.getMetadataMap().put(Pcp.Metadata.MessageTopic.name(), topic);
            outboundBuilder.getMetadataMap().put(Pcp.Metadata.InstanceId.name(), instanceId);
            outboundBuilder.getMetadataMap().put(Pcp.Metadata.Timestamp.name(),Long.toString( transferQueueApplyInfo.getApplyTimestamp()));
    ...
    public  Firework.ApplyTransferQueueResponse  applyFromMaster(Firework.ApplyTransferQueueRequest produceRequest){
        if(!isMaster()){
            ManagedChannel managedChannel = GrpcConnectionFactory.createManagedChannel(getMasterAddress());
