...
def __compute_partition_gradient(data, embed, fit_intercept=True):
    ...
    gs = []
    for x, vx_fg, x_square_fg, f_g in zip(feature, vx_mul_fg, x_square_mul_fg, fore_gradient):
        g1 = np.multiply(x, np.expand_dims(vx_fg, 1))
        g2 = np.multiply(embed.T, x_square_fg)
        gradient_v = g1 - g2
        gs.append(gradient_v.flatten())
...
def compute_gradient(data_instances, fore_gradient, vx_mul_fg, x_square_mul_fg, model_weights):
    ...
    """
    feat_join_grad = data_instances.join(fore_gradient, lambda d, g: (d.features, g[0]))
...
class Guest(BaseFM, loss_sync.Guest):
    ...
    def compute_gradient_procedure(self, data_instances, encrypted_calculator, model_weights, optimizer,
        ...
        self.guest_forward, self.raw_guest_forward = self.compute_forwards(data_instances, model_weights, encrypted_calculator, batch_index)
        ...
        # send to host: (fore_gradient, fore_gradient * guest_vx)
        fore_gradient = fore_gradient.join(self.guest_forward, lambda a, b: (a, np.multiply(a, b[1])))
        self.remote_fore_gradient(fore_gradient, suffix=current_suffix)

        # receive vx_mul_fg from host
        agg_vx_mul_fg = self.get_agg_vx_mul_fg(suffix=current_suffix)
        assert(len(agg_vx_mul_fg) == 1, "fm only support single host")
        agg_vx_mul_fg = agg_vx_mul_fg[0]

        # compute guest gradient and loss
        x_square = data_instances.mapValues(lambda v: np.multiply(v.features, v.features))
        en_x_square_mul_fg = x_square.join(fore_gradient, lambda xs, fg: xs * fg[0])
    ...
    def get_agg_vx_mul_fg(self, suffix=tuple()):
        agg_vx_mul_fg = self.agg_vx_mul_fg_transfer.get(idx=-1, suffix=suffix)
...
class Host(BaseFM, loss_sync.Host):
    ...
    def compute_gradient_procedure(self, data_instances, model_weights,
        ...
        # receive from guest: (fore_gradient, fore_gradient * guest_vx)
        ...
        # en_x_square = encrypted_calculator[batch_index].encrypt(x_square)
        agg_vx_mul_fg = host_vx.join(fore_gradient, lambda hvx, fg: hvx * fg[0] + fg[1])

        # send to guest, for gradient compute
        self.remote_vx_mul_fg(agg_vx_mul_fg, suffix=current_suffix)
    ...
    def remote_vx_mul_fg(self, agg_vx_mul_fg, suffix=tuple()):
        self.agg_vx_mul_fg_transfer.remote(obj=agg_vx_mul_fg, role=consts.GUEST, idx=0, suffix=suffix)
    ...
    def compute_forwards(self, data_instances, model_weights, encrypted_calculator):
        # w*x + <vi,vj>xi*xj + b
        fm_forward = self.compute_fm(data_instances, model_weights)
