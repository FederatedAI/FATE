...
class NCFInitParam(InitParam):
    def __init__(self, embed_dim=10, init_method='random_normal'):
        super(NCFInitParam, self).__init__()
    ...
    def check(self):
        if type(self.embed_dim).__name__ not in ["int"] or self.embed_dim < 0:
            raise ValueError(
                "NCFInitParam's embed_dim {} not supported, should be 'int'"
                "and greater than 0".format(
                    self.embed_dim))
        return True


class NCFParam(BaseParam):
    """
    ...
    mlp_params: dict, default {}, mlp layer parameters
    ...
    def check(self):
        ...
        if 'learning_rate' in self.optimizer.__dict__["kwargs"] and \
            ...
            raise ValueError(
                "neural_cf's optimizer['learning_rate'] {} not supported, should be float type and greater "
        ...
        if 'decay' in self.optimizer.__dict__["kwargs"] and \
            ...
            raise ValueError(
                "neural_cf's optimizer['decay'] {} not supported, should be float type and greater "
        ...
        if type(self.max_iter).__name__ != "int":
            raise ValueError(
                "neural_cf's max_iter {} not supported, should be int type".format(self.max_iter))
        elif self.max_iter <= 0:
            raise ValueError(
                "neural_cf's max_iter must be greater or equal to 1")
...
class HeteroNCFParam(NCFParam):
    ...
    def __init__(self, optimizer={"optimizer": "SGD", "learning_rate": 0.01},
        ...
        super(HeteroNCFParam, self).__init__(optimizer=optimizer,
    ...
    def check(self):
        ...
        if not isinstance(self.aggregate_iters, int):
            raise ValueError(
                "neural_cf's aggregate_iters {} not supported, should be int type".format(
                    self.aggregate_iters))

        return True

    def generate_pb(self):
        pb = ncf_model_meta_pb2.HeteroNCFParam()
        ...
        pb.mlp_params = json.dumps(self.mlp_params)
        pb.loss = self.loss
