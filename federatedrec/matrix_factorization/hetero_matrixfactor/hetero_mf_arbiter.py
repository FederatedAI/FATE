...
class HeteroMFArbiter(HeteroMFBase):
    """
    Third party coordinator of federated training process.
    """
    def __init__(self):
        super(HeteroMFArbiter, self).__init__()
        self.role = consts.ARBITER
        self.loss_consumed = None
    ...
    def _init_model(self, params):
        ...
        self.converge_func = converge_func_factory(
            early_stop.converge_func, early_stop.eps).is_converge
    ...
    def _check_monitored_status(self):
        ...
        if self.loss_consumed:
            ...
            return self.aggregator.send_converge_status(
                self.converge_func, converge_args=converge_args, suffix=self._iter_suffix())
        return None
