...
def main(config="../../config.yaml", namespace=""):
    ...
    # define guest model
    ReLU, Linear = nn.ReLU, nn.Linear

    guest_bottom_a = Linear(9, 3, True)
    seq = nn.Sequential(
        OrderedDict([
            ('layer_1', guest_bottom_a),
            ('relu_1', ReLU())
        ])
    )
    ...
    # define interactive layer
    interactive_layer = Linear(3, 2, True)
    ...
    init.xavier_normal_(seq3)
    init.xavier_normal_(interactive_layer)

    # loss function
    ce_loss_fn = nn.CrossEntropyLoss()

    # optimizer, after applying fate torch hook, optimizer can be created without parameters
