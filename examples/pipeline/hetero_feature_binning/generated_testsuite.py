...
def insert_extract_code(file_path, fold_name):
    ...
    f_str = f_str.replace('common_tools.prettify(pipeline.get_component("hetero_feature_binning_0").get_summary())',
                          '')
    return f_str
...
def do_generated(fold_name='hetero_feature_binning'):
    folder = '.'
    files = os.listdir(".")
    cmd = 'python {}'
    ...
    suite_json = get_testsuite_file('hetero_feature_binning/hetero_binning_testsuite.json')
    conf_files = os.listdir('./{}/'.format(generated_path))
    f_dsl = {f.split('_')[-2]: f for f in conf_files if 'dsl.json' in f}
    f_conf = {f.split('_')[-2]: f for f in conf_files if 'conf.json' in f}
