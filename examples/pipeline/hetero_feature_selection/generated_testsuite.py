...
def insert_extract_code(file_path, fold_name):
    ...
    f_str = f_str.replace('common_tools.prettify(pipeline.get_component("hetero_feature_selection_0").get_summary())',
                          '')
    return f_str
...
def do_generated(fold_name='hetero_feature_selection'):
    folder = '.'
    files = os.listdir(".")
    cmd = 'python {}'
    ...
    suite_json = get_testsuite_file('hetero_feature_selection/hetero_feature_selection_testsuite.json')
    conf_files = os.listdir('./{}/'.format(generated_path))
    f_dsl = {"-".join(f.split('_')[3: -1]): f for f in conf_files if 'dsl.json' in f}
    f_conf = {"-".join(f.split('_')[3: -1]): f for f in conf_files if 'conf.json' in f}
