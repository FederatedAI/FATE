...
def main(config="../../config.yaml", namespace=""):
    ...
    reader_0.get_party_instance(role="host", party_id=hosts[0]).algorithm_param(table=host_train_data_0)
    reader_0.get_party_instance(role="host", party_id=hosts[1]).algorithm_param(table=host_train_data_1)

    reader_1.get_party_instance(role="guest", party_id=guest).algorithm_param(table=guest_validate_data)
    reader_1.get_party_instance(role="host", party_id=hosts[0]).algorithm_param(table=host_validate_data_0)
    reader_1.get_party_instance(role="host", party_id=hosts[1]).algorithm_param(table=host_validate_data_1)
    ...
    dataio_1.get_party_instance(role="guest", party_id=guest).algorithm_param(with_label=True, output_format="dense",
                                                                              label_name="motor_speed", label_type="float")
    dataio_1.get_party_instance(role="host", party_id=hosts[0]).algorithm_param(with_label=False)
    dataio_1.get_party_instance(role="host", party_id=hosts[1]).algorithm_param(with_label=False)
