...
from fate_client.pipeline.components.fate import CoordinatedPoisson, PSI, Evaluation
...
def main(config="../config.yaml", namespace=""):
    ...
    pipeline.deploy([psi_0, poisson_0])

    predict_pipeline = FateFlowPipeline()

    deployed_pipeline = pipeline.get_deployed_pipeline()
    deployed_pipeline.psi_0.guest.component_setting(
        input_data=DataWarehouseChannel(name="dvisits_hetero_guest",
                                        namespace=f"experiment{namespace}"))
    deployed_pipeline.psi_0.hosts[0].component_setting(
        input_data=DataWarehouseChannel(name="dvisits_hetero_host",
                                        namespace=f"experiment{namespace}"))
