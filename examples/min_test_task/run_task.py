...
def exec_task(config_dict, task, role):
    ...
    if status != 0:
        raise ValueError("[exec_task] task:{}, role:{} exec fail, status:{}, stdout:{}".format(task, role, status, stdout))
...
def download(config_file,scene_id, guest_id, host_id, arbiter_id, role, data_type):
    ...
    parse_result = parse_exec_task(stdout)
    print("[Task Download]finsih download data_type:{}, table_name:{}, namespace:{}".format(data_type, parse_result["table_name"], parse_result["namespace"]))
    return parse_result["table_name"], parse_result["namespace"]
...
def download_id_library(config_file,scene_id, guest_id, host_id, role):
    ...
    stdout = exec_task(json_info, "download", role)
    parse_result = parse_exec_task(stdout)
    print("[Task Download]finsih download id libraray data_type:data_input, table_name:{}, namespace:{}".format(parse_result["table_name"], parse_result["namespace"]))
    return parse_result["table_name"], parse_result["namespace"]
...
def workflow_job_status_checker(jobid):
    ...
    while not check_data:
        time.sleep(2)
        print("[Workflow_Job_Status_checker] retry check jobid:{} status, current retry counter:{}".format(jobid, retry_counter))
        stdout = job_status_checker(jobid)
        check_data = stdout["data"]
        retry_counter += 1
        if retry_counter >= 5:
...
def intersect(config_file, guest_id, host_id,scene_id, 
    ...
    json_info["role_parameters"]["guest"]["WorkFlowParam"]["intersect_data_output_namespace"] = intersect_output_namespace
...
if __name__ == "__main__":
    ...
    if method == "upload":
        ...
        if target == "intersect":
            table_name, table_namespace= import_id_library(import_config_file, scene_id, guest_id, host_id, role, data_file)
        else:
            table_name, table_namespace = upload(upload_config_file, scene_id, guest_id, host_id, arbiter_id, role, data_file, data_type)
        ...
        try:
            ...
            guest_table_name, guest_namespace = download(download_config_file, scene_id, guest_id, host_id, arbiter_id, "guest", "data_input")

        # job_status = intersect(intersect_config_file, guest_id, host_id, scene_id)
        job_status = intersect(intersect_config_file, guest_id, host_id, scene_id, guest_name=guest_table_name, guest_namespace=guest_namespace)
        ...
        if job_status is SUCCESS:
            ...
            eval_out_name, eval_out_namespace = download(download_config_file, scene_id, guest_id, host_id, arbiter_id, "guest", "evaluation_output")
        ...
        guest_table_name, guest_namespace = download_id_library(download_id_library_config_file, scene_id, guest_id, host_id, "guest")
        count = get_table_count(guest_table_name, guest_namespace)
        if count != task_data_count:
            TEST_TASK["TEST_UPLOAD"] = 1
            raise ValueError("[failed] Test upload intersect task error, upload data count is:{}, it should be:{}".format(count, task_data_count))
        else:
        ...
        for data_type in test_data_type:
            table_name, table_namespace = download(download_config_file, scene_id, guest_id, host_id, arbiter_id, "guest", data_type)
            count = get_table_count(table_name, table_namespace)
            if count != task_data_count:
                TEST_TASK["TEST_UPLOAD"] = 1
                raise ValueError("[failed] Test upload {} task error, upload data count is:{}, it should be:{}".format(data_type, count, task_data_count))
            else:
        ...
        if job_status is SUCCESS:
            ...
            if count != task_intersect_count:
                ...
                raise ValueError("[failed] Test intersect task error, intersect output count is:{}, it should be:{}".format(count, task_data_count))
            else:
