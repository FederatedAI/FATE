...
from fate_flow.settings import END_STATUS_JOB_SCHEDULING_TIME_LIMIT, END_STATUS_JOB_SCHEDULING_UPDATES, WORK_MODE
...
class DAGScheduler(Cron):
    ...
    def submit(cls, job_data, job_id=None):
        ...
        schedule_logger(job_id).info(
            'submit job, job_id {}, body {}'.format(job_id, job_data))
        ...
        if job.f_initiator_party_id not in job_runtime_conf['role'][job.f_initiator_role]:
            schedule_logger(job_id).info(
                "initiator party id error:{}".format(job.f_initiator_party_id))
        ...
        JobController.adapt_job_parameters(
            role=job.f_initiator_role, job_parameters=common_job_parameters, create_initiator_baseline=True)
        ...
        if common_job_parameters.backend != StandaloneBackend.STANDALONE_PURE:
            for role, party_ids in job.f_roles.items():
                for party_id in party_ids:
                    ...
                    JobController.initialize_tasks(
                        job_id, role, party_id, False, job.f_initiator_role, job.f_initiator_party_id, common_job_parameters, dsl_parser)
    ...
    def run_do(self):
        ...
        jobs = JobSaver.query_job(
            is_initiator=True, status=JobStatus.WAITING, order_by="create_time", reverse=False)
        ...
        jobs = JobSaver.query_job(
            is_initiator=True, status=JobStatus.RUNNING, order_by="create_time", reverse=False)
        ...
        for job in jobs:
            ...
            try:
                ...
                schedule_logger(job.f_job_id).exception(e)
                schedule_logger(job.f_job_id).error(
                    f"schedule job {job.f_job_id} failed")
        ...
        jobs = JobSaver.query_job(
            is_initiator=True, ready_signal=True, order_by="create_time", reverse=False)
        ...
        jobs = JobSaver.query_job(
            is_initiator=True, rerun_signal=True, order_by="create_time", reverse=False)
        ...
        jobs = JobSaver.query_job(is_initiator=True, status=set(EndStatus.status_list()), end_time=[
                                  current_timestamp() - END_STATUS_JOB_SCHEDULING_TIME_LIMIT, current_timestamp()])
    ...
    def schedule_waiting_jobs(cls, job):
        ...
        try:
            ...
            if apply_status_code == FederatedSchedulingStatusCode.SUCCESS:
                cls.start_job(job_id=job_id, initiator_role=initiator_role,
                ...
                schedule_logger(job_id).info("job {} apply resource failed on {}, rollback {}".format(
                    job_id,
                    ",".join([",".join([f"{_r}:{_p}" for _p in _ps])
                ...
                if rollback_party:
                    return_status_code, federated_response = FederatedScheduler.resource_for_job(
                        job=job, operation_type=ResourceOperation.RETURN, specific_dest=rollback_party)
                ...
                if apply_status_code == FederatedSchedulingStatusCode.ERROR:
                    cls.stop_job(job_id=job_id, role=initiator_role,
                                 party_id=initiator_party_id, stop_status=JobStatus.FAILED)
                    schedule_logger(job_id).info(
    ...
    def schedule_ready_job(cls, job):
        job_id, initiator_role, initiator_party_id, = job.f_job_id, job.f_initiator_role, job.f_initiator_party_id
        update_status = cls.ready_signal(
            job_id=job_id, set_or_reset=False, ready_timeout_ttl=60 * 1000)
        schedule_logger(job_id).info(
            f"reset job {job_id} ready signal {update_status}")
    ...
    def schedule_rerun_job(cls, job):
        if EndStatus.contains(job.f_status):
            ...
            if status == FederatedSchedulingStatusCode.SUCCESS:
                ...
                FederatedScheduler.sync_job(job=job, update_fields=[
                                            "ready_signal", "ready_time", "rerun_signal", "progress", "end_time", "elapsed"])
                schedule_logger(job_id=job.f_job_id).info(
    ...
    def start_job(cls, job_id, initiator_role, initiator_party_id):
        schedule_logger(job_id=job_id).info("try to start job {} on initiator {} {}".format(
        ...
        if jobs:
            ...
            schedule_logger(job_id=job_id).info("start job {} on initiator {} {}".format(
                job_id, initiator_role, initiator_party_id))
        else:
            schedule_logger(job_id=job_id).error("can not found job {} on initiator {} {}".format(
                job_id, initiator_role, initiator_party_id))

    @classmethod
    def schedule_running_job(cls, job):
        schedule_logger(job_id=job.f_job_id).info(
            "scheduling job {}".format(job.f_job_id))
        ...
        new_job_status = cls.calculate_job_status(
            task_scheduling_status_code=task_scheduling_status_code, tasks_status=tasks_status)
        ...
        schedule_logger(job_id=job.f_job_id).info(
            "Job {} status is {}, calculate by task status list: {}".format(job.f_job_id, new_job_status, tasks_status))
        if new_job_status != job.f_status or new_progress != job.f_progress:
            ...
            if int(new_progress) - job.f_progress > 0:
                ...
                FederatedScheduler.sync_job(
                    job=job, update_fields=["progress"])
                cls.update_job_on_initiator(
                    initiator_job=job, update_fields=["progress"])
            if new_job_status != job.f_status:
                ...
                FederatedScheduler.sync_job_status(job=job)
                cls.update_job_on_initiator(
                    initiator_job=job, update_fields=["status"])
        if EndStatus.contains(job.f_status):
    ...
    def rerun_job(cls, job_id, initiator_role, initiator_party_id, component_name):
        ...
        if component_name != job_utils.job_virtual_component_name():
            tasks = JobSaver.query_task(
                job_id=job_id, role=initiator_role, party_id=initiator_party_id, component_name=component_name)
        else:
            tasks = JobSaver.query_task(
                job_id=job_id, role=initiator_role, party_id=initiator_party_id)
        job_can_rerun = False
        ...
        for task in tasks:
            if task.f_status in {TaskStatus.WAITING, TaskStatus.SUCCESS}:
                ...
                FederatedScheduler.stop_task(
                    job=job, task=task, stop_status=TaskStatus.CANCELED)
                FederatedScheduler.clean_task(
                    job=job, task=task, content_type="metrics")
                ...
                for _role, _party_ids in job.f_runtime_conf_on_party["role"].items():
                    for _party_id in _party_ids:
                        if _role == initiator_role and _party_id == initiator_party_id:
                            continue
                        JobController.initialize_tasks(job_id, _role, _party_id, False, job.f_initiator_role, job.f_initiator_party_id, RunParameters(
                            **job.f_runtime_conf_on_party["job_parameters"]), dsl_parser, component_name=task.f_component_name, task_version=task.f_task_version)
                schedule_logger(job_id=job_id).info(
                    f"create task {task.f_task_id} new version {task.f_task_version} successfully")
                job_can_rerun = True
        if job_can_rerun:
            schedule_logger(job_id=job_id).info(
                f"job {job_id} set rerun signal")
    ...
    def calculate_job_status(cls, task_scheduling_status_code, tasks_status):
        ...
        if len(tmp_status_set) == 1:
            ...
            raise Exception(
                "Calculate job status failed: {}".format(tasks_status))
    ...
    def stop_job(cls, job_id, role, party_id, stop_status):
        ...
        jobs = JobSaver.query_job(
            job_id=job_id, role=role, party_id=party_id, is_initiator=True)
        if len(jobs) > 0:
            if stop_status == JobStatus.CANCELED:
                schedule_logger(job_id=job_id).info(f"cancel job {job_id}")
                set_cancel_status = cls.cancel_signal(
                    job_id=job_id, set_or_reset=True)
                schedule_logger(job_id=job_id).info(
            ...
            if status_code == FederatedSchedulingStatusCode.SUCCESS:
                ...
                for initiator_task in initiator_tasks_group.values():
                    TaskScheduler.collect_task_of_all_party(
                        job, initiator_task=initiator_task, set_status=stop_status)
                schedule_logger(job_id=job_id).info(
    ...
    def ready_signal(cls, job_id, set_or_reset: bool, ready_timeout_ttl=None):
        ...
        if set_or_reset:
            update_fields = {Job.f_ready_signal: True,
                             Job.f_ready_time: current_timestamp()}
            ...
            if ready_timeout_ttl:
                filters.append(current_timestamp() -
                               Job.f_ready_time > ready_timeout_ttl)
    ...
    def cancel_signal(cls, job_id, set_or_reset: bool):
        update_status = Job.update({Job.f_cancel_signal: set_or_reset, Job.f_cancel_time: current_timestamp()}).where(
            Job.f_job_id == job_id).execute() > 0
        return update_status
    ...
    def rerun_signal(cls, job_id, set_or_reset: bool):
        if set_or_reset is True:
            update_fields = {Job.f_rerun_signal: True,
                             Job.f_cancel_signal: False, Job.f_end_scheduling_updates: 0}
        ...
        update_status = Job.update(update_fields).where(
            Job.f_job_id == job_id).execute() > 0
        return update_status
    ...
    def finish(cls, job, end_status):
        schedule_logger(job_id=job.f_job_id).info(
            "Job {} finished with {}, do something...".format(job.f_job_id, end_status))
        cls.stop_job(job_id=job.f_job_id, role=job.f_initiator_role,
                     party_id=job.f_initiator_party_id, stop_status=end_status)
