...
class LinkisSparkEngine(EngineABC):
    def run(self, task: Task, run_parameters, run_parameters_path, config_dir, log_dir, cwd_dir, **kwargs):
        linkis_execute_url = "http://{}:{}{}".format(ServiceRegistry.LINKIS_SPARK_CONFIG.get("host"),
                                                     ServiceRegistry.LINKIS_SPARK_CONFIG.get("port"),
                                                     LINKIS_EXECUTE_ENTRANCE)
        headers = {"Token-Code": ServiceRegistry.LINKIS_SPARK_CONFIG.get("token_code"),
    ...
    @staticmethod
    def kill(task):
        linkis_query_url = "http://{}:{}{}".format(ServiceRegistry.LINKIS_SPARK_CONFIG.get("host"),
                                                   ServiceRegistry.LINKIS_SPARK_CONFIG.get("port"),
