...
class GHPacker(object):
    ...
    def __init__(self, sample_num: int, en_calculator: EncryptModeCalculator,
                       precision=2**53, max_sample_weight=1.0, task_type=consts.CLASSIFICATION):
    ...
    def pack_and_encrypt(self, gh):
        ...
        fixed_int_encode_func = functools.partial(self.to_fixedpoint, mul=precision, g_offset=self.g_offset)
...
class PackedGHCompressor(object):
    ...
    def __init__(self):
        self.compressor = CipherCompressorHost(package_class=SplitInfoPackage)
