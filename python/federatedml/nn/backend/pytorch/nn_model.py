...
def build_optimzer(optim, model):
            # self.optimizer = str({'name': 'Adam', 'learning_rate': 0.001, 'decay': 0.0, 'beta_1': 0.9, 'beta_2': 0.999, 'epsilon': 1e-07, 'amsgrad': False})
    #TODO
    LOGGER.info(optim['optimizer'])
    optim = optim['optimizer']
    optim = json.loads(optim)
    return torch.optim.Adam(model.parameters(), lr=optim['learning_rate'], betas=(optim['beta_1'], optim['beta_2']), eps=optim['epsilon'], weight_decay=optim['decay'], amsgrad=False if optim['amsgrad']=='False' else True)
...
class PytorchNNModel(NNModel):
    def __init__(self, model, optimizer=None, loss=None, metrics=None):
        self._model: torch.nn.ModuleList = model
    ...
    def train(self, data: data.Dataset, **kwargs):
        ...
        optimizer = build_optimzer(self._optimizer.__dict__, self._model)
        epochs = 1
...
class PytorchData(data.Dataset):
    def __init__(self, data_instances, batch_size, encode_label, label_mapping):
        ...
        LOGGER.info("x_shape:"+str(self.x_shape))
        LOGGER.info("size:"+str(self.size))
...
class PytorchGraphData():
    def __init__(self, edges):
        ...
        index = 0
        self._keys = []
        nodes1 = np.zeros(size)
        nodes2 = np.zeros(size)
        edge_values = np.zeros(size)
        for k, v in edges.collect():
            self._keys.append(k)
            nodes1[index] = int(v.features[0])
            nodes2[index] = int(v.features[1])
            edge_values[index] = float(v.features[2])
            index += 1

        maxnode = int(max(max(nodes1), max(nodes2)))
        self.edges = torch.sparse_coo_tensor([nodes1, nodes2], edge_values, (maxnode+1, maxnode+1), dtype=torch.float32)
...
class PytorchGNNModel(NNModel):
    ...
    def train(self, data: data.Dataset, adj: data.Dataset, **kwargs):
        loss_fn = build_loss_fn(self._loss)
        # self.optimizer = str({'name': 'Adam', 'learning_rate': 0.001, 'decay': 0.0, 'beta_1': 0.9, 'beta_2': 0.999, 'epsilon': 1e-07, 'amsgrad': False})
        optimizer = build_optimzer(self._optimizer.__dict__, self._model)
        epochs = 1
    ...
    def evaluate(self, feats, adj, **kwargs):
        metircs = {}
        loss_metircs = []
        loss_fuc = []
        other_metrics = []
        ...
        self._model.eval()
        loss_fn = build_loss_fn(self._loss)
        evaluate_data = DataLoader(feats, batch_size=feats.batch_size, shuffle=False)
        if loss_metircs:
            loss_metircs_result = [0 for i in range(len(loss_metircs))]
        ...
        metircs["loss"] = loss.item() * feats.batch_size / len(feats)
        if loss_metircs:
            i = 0
            for func in loss_metircs:
                metircs[func] = loss_metircs_result[i] * feats.batch_size / len(feats)
                i += 1
        num_output_units = result.shape[1]
        if len(other_metrics) > 0:
            if num_output_units == 1:
                ...
                if feats.use_one_hot:
                    ...
                    for i in range(len(feats)):
                        if result[i].argmax() == eval_label[i]:
                            acc += 1
                metircs["auccuray"] = acc / len(feats)
        return metircs

    def predict(self, feats, adj, **kwargs):
        predict_data = DataLoader(feats, batch_size=feats.batch_size, shuffle=False)
        for batch_id, (feature, label) in enumerate(predict_data):
            feature = torch.tensor(feature, dtype=torch.float32)
            # label = torch.tensor(label, dtype=torch.float32)
            x = feature
