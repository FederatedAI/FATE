from federatedml.nn.backend.fate_torch.base import FateTorchLayer, FateTorchLoss
...
class RNNBase(nn.modules.rnn.RNNBase, FateTorchLayer):
    ...
    def __init__(self, mode, input_size, hidden_size, num_layers=1, bias=True, batch_first=False, dropout=0.0, bidirectional=False, **kwargs):
        FateTorchLayer.__init__(self)
...
class Embedding(nn.modules.sparse.Embedding, FateTorchLayer):
    ...
    def __init__(self, num_embeddings, embedding_dim, padding_idx=None, max_norm=None, norm_type=2.0, scale_grad_by_freq=False, sparse=False, _weight=None, **kwargs):
        FateTorchLayer.__init__(self)
...
class EmbeddingBag(nn.modules.sparse.EmbeddingBag, FateTorchLayer):
    ...
    def __init__(self, num_embeddings, embedding_dim, max_norm=None, norm_type=2.0, scale_grad_by_freq=False, mode='mean', sparse=False, _weight=None, **kwargs):
        FateTorchLayer.__init__(self)
        self.param_dict['max_norm'] = max_norm
        self.param_dict['norm_type'] = norm_type
        self.param_dict['scale_grad_by_freq'] = scale_grad_by_freq
...
class MultiheadAttention(nn.modules.activation.MultiheadAttention, FateTorchLayer):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, **kwargs):
        FateTorchLayer.__init__(self)
        self.param_dict['dropout'] = dropout
        self.param_dict['bias'] = bias
...
class _ConvNd(nn.modules.conv._ConvNd, FateTorchLayer):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation, transposed, output_padding, groups, bias, padding_mode, **kwargs):
        FateTorchLayer.__init__(self)
        self.param_dict['in_channels'] = in_channels
        self.param_dict['out_channels'] = out_channels
        self.param_dict['kernel_size'] = kernel_size
...
class Transformer(nn.modules.transformer.Transformer, FateTorchLayer):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=2048, dropout=0.1, activation='relu', custom_encoder=None, custom_decoder=None, **kwargs):
        FateTorchLayer.__init__(self)
...
class SyncBatchNorm(nn.modules.batchnorm.SyncBatchNorm, FateTorchLayer):
    ...
    def __init__(self, num_features, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True, process_group=None, **kwargs):
        FateTorchLayer.__init__(self)
        self.param_dict['eps'] = eps
        self.param_dict['momentum'] = momentum
        self.param_dict['affine'] = affine
