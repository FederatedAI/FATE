...
class TrainerBase(object):
    ...
    @staticmethod
    def task_type_infer(predict_result: t.Tensor):
    ...
    def format_predict_result(self, sample_ids: List, predict_result: t.Tensor,
        ...
        if task_type is None:
            task_type = self.task_type_infer(predict_result)
            if task_type is None:
                LOGGER.warning('unable to infer predict result type, predict process will be skipped')
                return None
        else:
            assert task_type in [consts.BINARY, consts.MULTY, consts.REGRESSION]
    ...
    def evaluation(self, sample_ids: list, pred_scores: t.Tensor, label: t.Tensor, dataset_type='train', metric_list=None, epoch_idx=0):

        eval_obj = Evaluation()
        task_type = self.task_type_infer(pred_scores)
