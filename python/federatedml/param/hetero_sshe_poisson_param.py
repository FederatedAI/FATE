...
class HeteroSSHEPoissonParam(LinearModelParam):
    """
    Parameters used for Hetero SSHE Poisson Regression.
    ...
    reveal_strategy: str, default: "respectively"
    ...
    def __init__(self, penalty='L2',
        ...
        super(HeteroSSHEPoissonParam, self).__init__(penalty=penalty, tol=tol, alpha=alpha, optimizer=optimizer,
    ...
    def check(self):
        ...
        if self.exposure_colname is not None:
            if type(self.exposure_colname).__name__ != "str":
                raise ValueError(
                    descr + f"exposure_colname {self.exposure_colname} not supported, should be string type")
        return True
