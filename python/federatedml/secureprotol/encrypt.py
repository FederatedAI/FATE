...
class IpclPaillierEncrypt(Encrypt):
    ...
    def raw_encrypt(self, plaintext, exponent=0):
        ...
        Encrypt without applying obfuscator.
            ...
            Returns:
                encryted_num (PaillierEncryptedNumber from `ipcl_python`)
        """
    ...
    def encrypt_list(self, values):
        """Encrypt a list of values into one ciphertext."""
        return self.encrypt(values)

    def decrypt_list(self, values):
        """Decrypt the input value (numpy.ndarray), which contains one ciphertext generated by `encrypt_list`."""
