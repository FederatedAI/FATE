...
# from python.fate_script.contract.secureprotol.fate_paillier import *
# from python.fate_script.contract.secureprotol.fixedpoint import *
...
GPU_LIB = cdll.LoadLibrary(os.path.dirname(__file__) + "/../../../C/GPU/c2048/GPU_2048.so")
...
def get_GPU_info():
    ...
    for i in range (pynvml.nvmlDeviceGetCount()):
        ...
        if i == get_GPU_device():
            print("Device id: {} -> Name: {}, Total: {} MB, Free: {} MB, Used: {} MB".format(
                i, name.decode(), mem_info.total / 2 ** 20, mem_info.free / 2 ** 20, mem_info.used / 2 ** 20))
        free_list.append(mem_info.free)
    pynvml.nvmlShutdown()
    return np.asarray(free_list)
...
def fp_c2bytes(store, res):
    ...
    # C memory storage
    bytes_res = c_buffer((PLAIN_BYTE + U_INT32_BYTE * 2) * store.vec_size +
                            U_INT32_BYTE * 2 + PLAIN_BYTE * 2)
...
def pi_c2bytes(store, res):
    ...
    # C memory storage
    bytes_res = c_buffer((CIPHER_BYTE + U_INT32_BYTE * 2) * store.vec_size +
                            U_INT32_BYTE * 2 + CIPHER_BYTE * 2)
...
def _te_init_ss(res_store, res_data, vec_size,
    ...
    '''
    return _te_init_store(res_store, res_data, vec_size,
                          mem_type, data_type), \
...
def _pi_init_ss(res_store, pen_storage, base_storage, exp_storage, vec_size,
    ...
    '''
    return _pi_init_store(res_store, pen_storage, base_storage, exp_storage,
                          vec_size, mem_type, data_type,
                          encode_n, encode_max_int), \
...
def _bi_init_ss(res_store, res_data, vec_size, res_shape, res_shape_tuple,
    ...
    return _bi_init_store(res_store, res_data, vec_size,
                          elem_size, mem_type), \
...
def _fp_init_ss(res_store, fpn_storage, base_storage, exp_storage,
    ...
    return _fp_init_store(res_store, fpn_storage, base_storage, exp_storage,
                          vec_size, n, max_int, mem_type, data_type), \
...
def __shape_resolve(shape_1, shape_2):
    ...
    def check_func(a, b): return a == b or a == 1 or b == 1
...
def pi_sum(pub_key, left_store, left_shape, axis,
    ...
    if len(left_shape_tuple) == 0:
        ...
        if len(left_shape_tuple) == 1 and axis != None and axis >= 1:
