...
def compute_predict_details(dataframe: Union[pd.DataFrame, DataFrame], task_type, classes: list = None, threshold=0.5):
    ...
    if task_type == BINARY:
        ...
        if len(classes) > 2:
            ...
            predict_details = [predict_detail_dict_to_str({classes[j]: float(pred[i][j]) for j in range(len(classes))}) for i in range(len(pred))]
        else:
