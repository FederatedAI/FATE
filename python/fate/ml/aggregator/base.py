...
class BaseAggregatorClient(Aggregator):
    ...
    def __init__(self, ctx: Context, aggregator_name: str = None, aggregate_type='mean', sample_num=1, is_mock=True, require_grad=True, float_p='float64') -> None:
    ...
    def _process_model(self, model):
        ...
        if isinstance(model, t.nn.Module):
            ...
            if self.require_grad:
                agg_list = [self._convert_type(p.cpu().detach().numpy(), self.float_p) for p in parameters if p.requires_grad]
            else:
...
class BaseAggregatorServer(Aggregator):
    ...
    def __init__(self, ctx: Context, aggregator_name: str = None, is_mock=True) -> None:
        ...
        self.aggregator_name = 'default' if aggregator_name is None else aggregator_name
        self.model_aggregator = sa_server(prefix=self.aggregator_name+'_model', is_mock=is_mock, ranks=[ctx.guest.rank, *ctx.hosts.ranks])
        self.loss_aggregator = sa_server(prefix=self.aggregator_name+'_loss', is_mock=is_mock, ranks=[ctx.guest.rank, *ctx.hosts.ranks])
