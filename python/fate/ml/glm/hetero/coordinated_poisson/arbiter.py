...
class CoordinatedPoissonModuleArbiter(HeteroModule):
    ...
    def fit(self, ctx: Context) -> None:
        ...
        if self.estimator is None:
            ...
            single_estimator = HeteroPoissonEstimatorArbiter(
                epochs=self.epochs,
    ...
    def from_model(cls, model):
        poisson_model = CoordinatedPoissonModuleArbiter(
            model["meta"]["epochs"],
        ...
        estimator = HeteroPoissonEstimatorArbiter()
        estimator.restore(model["data"]["estimator"])
        poisson_model.estimator = estimator
        return poisson_model


class HeteroPoissonEstimatorArbiter(HeteroModule):
    def __init__(
    ...
    def fit_model(self, ctx, decryptor):
        ...
        for i, iter_ctx in ctx.on_iterations.ctxs_range(self.epochs):
            ...
            for batch_ctx, _ in iter_ctx.on_batches.ctxs_zip(batch_loader):
                ...
                batch_ctx.hosts[i].put("g", delta_g_list[1])
                delta_g_list_squeezed.append(delta_g_list[1].squeeze())
                if iter_g is None:
            ...
            if iter_loss is not None:
                # logger.info(f"step={i}: poisson_loss={iter_loss.tolist()}")
                iter_ctx.metrics.log_loss("poisson_loss", iter_loss.tolist()[0])
