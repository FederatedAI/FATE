...
class HeteroPoissonModuleArbiter(HeteroModule):
    def __init__(
            self,
            max_iter,
            early_stop,
    ...
    def fit(self, ctx: Context) -> None:
        ...
        single_estimator = HeteroPoissonEstimatorArbiter(max_iter=self.max_iter,
    ...
    def from_model(cls, model):
        poisson = HeteroPoissonModuleArbiter(**model["metadata"])
    ...
    def fit_model(self, ctx, decryptor):
        ...
        for i, iter_ctx in ctx.range(self.start_iter, self.max_iter):
            ...
            if iter_loss is not None:
                iter_ctx.metrics.log_loss("poisson_loss", iter_loss.tolist(), step=i)
