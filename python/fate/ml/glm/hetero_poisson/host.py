...
class HeteroPoissonModuleHost(HeteroModule):
    def __init__(
            self,
            max_iter,
            batch_size,
    ...
    def fit(self, ctx: Context, train_data, validate_data=None) -> None:
        ...
        estimator = HeteroPoissonEstimatorHost(max_iter=self.max_iter,
    ...
    @classmethod
    def from_model(cls, model) -> "HeteroPoissonModuleHost":
        poisson = HeteroPoissonModuleHost(**model["metadata"])
        estimator = HeteroPoissonEstimatorHost()
...
class HeteroPoissonEstimatorHost(HeteroModule):
    def __init__(
            self,
            max_iter=None,
            batch_size=None,
    ...
    def fit_model(self, ctx: Context, encryptor, train_data, validate_data=None) -> None:
        ...
        for i, iter_ctx in ctx.range(self.max_iter):
            ...
            for batch_ctx, X in iter_ctx.iter(batch_loader):
                ...
                mu_h = torch.exp(Xw_h)
                encryptor.encrypt(mu_h).to(batch_ctx.guest, "mu_h")
