...
if __name__ == "__main__":
    ...
    if party == "guest":
        ...
        model = HeteroNNModelGuest(top_model=guest_top, agg_layer=AggLayerGuest(merge_type="concat"))
        model.double()
        optimizer = t.optim.Adam(model.parameters(), lr=0.01)

        args = TrainingArguments(num_train_epochs=5, per_device_train_batch_size=16, disable_tqdm=False, no_cuda=True)
        ...
        model = HeteroNNModelHost(bottom_model=host_bottom, agg_layer=AggLayerHost())
        optimizer = t.optim.Adam(model.parameters(), lr=0.01)

        args = TrainingArguments(num_train_epochs=5, per_device_train_batch_size=16, disable_tqdm=False, no_cuda=True)

        trainer = HeteroNNTrainerHost(ctx=ctx, model=model, optimizer=optimizer, train_set=dataset, training_args=args)
        trainer.train()
