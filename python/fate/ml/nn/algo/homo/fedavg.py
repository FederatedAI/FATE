...
class FedAVGCLient(FedTrainerClient):
    ...
    def init_aggregator(self, ctx: Context, fed_args: FedArguments):
        ...
        assert aggregator in {item.value for item in AggregatorType}, f"aggregator should be one of {{item.value for item in AggregatorType}}, but got {aggregator}"
        ...
        aggregator = client_class(ctx, aggregate_type=aggregate_type, aggregator_name=aggregator_name, sample_num=sample_num)
...
class FedAVGServer(FedTrainerServer):
    ...
    def on_federation(
            self,
            ctx: Context,
            aggregator: Union[SecureAggregatorServer, PlainTextAggregatorServer]):
