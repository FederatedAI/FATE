...
class HeteroDecisionTreeGuest(DecisionTree):
    ...
    def sync_nodes(self, ctx: Context, cur_layer_nodes: List[Node], next_layer_nodes: List[Node]):
    ...
    def booster_fit(self, ctx: Context, bin_train_data: DataFrame, grad_and_hess: DataFrame, bining_dict: dict):
        ...
        for cur_depth, sub_ctx in ctx.on_iterations.ctxs_range(self.max_depth):
            ...
            next_layer_nodes = self.update_tree(ctx, cur_layer_node, split_info)
            self.sync_nodes(sub_ctx, cur_layer_node, next_layer_nodes)
            ...
            self.sample_pos.append(sample_pos.as_pd_df())
