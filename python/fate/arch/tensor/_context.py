...
class Context:
    ...
    @overload
    def keygen(self, kind: Literal[CipherKind.PHE], key_length: int) -> Tuple["PHEEncryptor", "PHEDecryptor"]:
        ...
        ...
        if kind == CipherKind.PHE or kind == CipherKind.PHE_PAILLIER:
            if self.distributed == Distributed.NONE:
                if self.device == Device.CPU:
                    ...
                    encryptor, decryptor = PaillierPHECipherLocal().keygen(key_length=key_length)
            ...
            if self.distributed == Distributed.EGGROLL:
                if self.device == Device.CPU:
                    ...
                    encryptor, decryptor = PaillierPHECipherDistributed().keygen(key_length=key_length)
                    return PHEEncryptor(encryptor), PHEDecryptor(decryptor)
    ...
    def random_tensor(self, shape, num_partition=1) -> "FPTensor":
        if self.distributed == Distributed.NONE:
            ...
            return FPTensor(
                self,
                FPTensorDistributed(computing_session.parallelize(parts, include_key=False, partition=num_partition)),
            )
    ...
    def sub_namespace(self, namespace):
        ...
        # into subnamespace
        self._namespace_state = NamespaceState(self._namespace_state.sub_namespace(namespace))
    ...
    @overload
    @contextmanager
    def iter_namespaces(self, stop: int, *, prefix_name="") -> Generator[Generator["Context", None, None], None, None]:
        ...
