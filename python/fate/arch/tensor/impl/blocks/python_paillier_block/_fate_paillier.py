...
from ._fixedpoint import FixedPointNumber
from ._gmpy_math import getprimeover, invert, mpz, powmod
...
class PaillierPublicKey(object):
    ...
    def raw_encrypt(self, plaintext, random_value=None):
        ...
        if not isinstance(plaintext, int):
            raise TypeError("plaintext should be int, but got: %s" % type(plaintext))
...
class PaillierPrivateKey(object):
    ...
    def __init__(self, public_key, p, q):
        ...
        self.q_inverse = invert(self.q, self.p)
    ...
    def h_func(self, x, xsquare):
        ...
        return invert(self.l_func(powmod(self.public_key.g, x - 1, xsquare), x), x)
    ...
    def raw_decrypt(self, ciphertext):
        ...
        mp = (
            self.l_func(powmod(ciphertext, self.p - 1, self.psquare), self.p)
            * self.hp
            % self.p
        )

        mq = (
            self.l_func(powmod(ciphertext, self.q - 1, self.qsquare), self.q)
            * self.hq
            % self.q
        )
...
class PaillierEncryptedNumber(object):
    ...
    def __init__(self, public_key, ciphertext, exponent=0):
        ...
        if not isinstance(self.__ciphertext, int):
            raise TypeError(
                "ciphertext should be an int, not: %s" % type(self.__ciphertext)
            )
