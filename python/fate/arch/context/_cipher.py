...
class PHECipher:
    ...
    def keygen(self, options, **kwargs):
        from fate.arch.tensor import phe_keygen

        return phe_keygen(self.kind, options)

    def setup(self, key_length):
        from fate.arch.tensor import PHEKit

        return PHEKit(self.kind, key_length)
