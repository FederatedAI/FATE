...
class DistributedArithmeticSharedTensor(ArithmeticSharedTensor):
    """
    ...
    def __init__(
        ...
        self.share = DistributedArithmeticSharedTensor.PRZS_distributed(ctx, size, device=device)
        if self.rank == src:
            self.share += tensor
    ...
    @staticmethod
    def PRZS_distributed(ctx, shape, device=None):
        from fate.arch.tensor import DTensor

        return DTensor.from_sharding_list(ctx, [torch.rand(5, 11).long(), torch.rand(4, 11).long(), torch.rand(3, 11).long()], )
        raise NotImplementedError("PRZS_distributed is not implemented")
        ...
