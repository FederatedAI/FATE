...
from .ops import aggregate_indexer, extract_columns, transform_to_tensor, \
    transform_to_table, transform_list_to_block, transform_list_block_to_frame_block, get_partition_order_mappings
from .manager import SchemaManager, BlockManager, Schema


# TODO: record data type, support multiple data types
# TODO：filter等操作，如何快速shuffle并且重新聚合


class DataFrame(object):
    def __init__(self, ctx, block_table, partition_order_mappings, schema_manager: SchemaManager, block_manager: BlockManager):
        self._ctx = ctx
    ...
    def sample_id(self):
        if self._sample_id is None:
            self._sample_id = self.__extract_columns(with_sample_id=True,
                                                     with_match_id=False,
                                                     with_label=False,
                                                     with_weight=False)
    ...
    def match_id(self):
        if self._match_id is None:
            self._match_id = self.__extract_columns(with_sample_id=True,
                                                    with_match_id=True,
                                                    with_label=False,
                                                    with_weight=False)
    ...
    def label(self):
        if self._label is None:
            self._label = self.__extract_columns(
                with_sample_id=True,
                with_match_id=True,
                with_label=True,
                with_weight=False
    ...
    def weight(self):
        if self._weight is None:
            self._weight = self.__extract_columns(
                with_sample_id=True,
                with_match_id=True,
                with_label=False,
                with_weight=False
    ...
    def shape(self) -> "tuple":
        ...
        if self._sample_id_indexer:
            ...
            items = self._block_table.mapValues(lambda block: 0 if block is None else len(block[0])).reduce(lambda size1, size2: size1 + size2)

        self.__shape = (items, len(self.schema.columns))
    ...
    @property
    def columns(self):
        return self._schema_manager.schema.columns
    ...
    def create_frame(self, with_label=False, with_weight=False, columns: list = None) -> "DataFrame":
        return self.__extract_columns(with_sample_id=True,
                                      with_match_id=True,
    ...
    def __extract_columns(self, with_sample_id=True, with_match_id=True,
        ...
        schema_manager, derive_indexes = self._schema_manager.derive_new_schema_manager(with_sample_id=with_sample_id,
                                                                                        ...
                                                                                        with_weight=with_weight,
                                                                                        columns=columns)
        block_manager, block_retrieval_indexes = self._block_manager.derive_new_block_manager(derive_indexes)

        retrieval_block_table = extract_columns(self._block_table, block_retrieval_indexes)

        return DataFrame(
            self._ctx,
            retrieval_block_table,
    ...
    def __convert_to_tensor(self, columns: Union[str, list], dtype: str = None):
        ...
        column_indexes = [self._schema_manager.get_column_index(column) for column in columns]
        block_indexes = [self._block_manager.get_block_id(column) for column in column_indexes]
        _, block_retrieval_indexes = self._block_manager.derive_new_block_manager(column_indexes)
    ...
    def __convert_to_table(self, target_name):
        column = self._schema_manager.get_column_index(target_name)
        block_id = self._block_manager.get_block_id(column)
        assert block_id[1] == 0, "support only one indexer in current version"

        return transform_to_table(self._block_table, block_id[0], self._partition_order_mappings)
