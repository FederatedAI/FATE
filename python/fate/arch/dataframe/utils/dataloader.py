...
from fate.arch.tensor import GUEST, HOST, Arbiter
...
class DataLoader(object):
    def __init__(self, dataset, ctx=None, mode="homo", need_align=False, batch_size=-1, shuffle=False, batch_strategy="full", random_seed=None):
        ...
        self._batch_size = min(batch_size, len(dataset)) if batch_size != -1 else len(dataset)
...
class FullBatchDataLoader(object):
    ...
    def _prepare(self):
        ...
        if self._batch_size == len(self._dataset):
            ...
            if self._mode in ["homo", "local"] or self._ctx.is_guest:
                ...
                for i in range(self._batch_num):
                    ...
                    sub_frame = self._dataset.loc(indexes, repartition=True)

                    if self._ctx.is_guest:
                        self._ctx.push(HOST, "batch_indexes", batch_indexes)

                    self._batch_splits.append(sub_frame)
            elif self._mode == self._ctx.is_host:
                batch_indexes = self._ctx.pull(GUEST, "batch_indexes")
