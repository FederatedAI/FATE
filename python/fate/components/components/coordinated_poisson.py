...
from fate.ml.glm import CoordinatedPoissonModuleArbiter, CoordinatedPoissonModuleGuest, CoordinatedPoissonModuleHost

logger = logging.getLogger(__name__)


@cpn.component(roles=[GUEST, HOST, ARBITER], provider="fate")
def coordinated_poisson(ctx, role):
    ...


@coordinated_poisson.train()
def train(
        ctx: Context,
        role: Role,
        ...
        exposure_col: cpn.parameter(type=str, default=None, desc="exposure column name"),
    ...
    logger.info(f"enter coordinated poisson train")
    # temp code start
    ...
    if role.is_guest:
        train_guest(
            ...
            batch_size, optimizer, learning_rate_scheduler, init_param, warm_start_model, exposure_col
        )
    elif role.is_host:
        train_host(
            ctx, train_data, validate_data, train_output_data, output_model, epochs,
...
@coordinated_poisson.cross_validation()
def cross_validation(
    ...
    if role.is_arbiter:
        ...
        for fold_ctx, _ in ctx.on_cross_validations.ctxs_zip(zip(range(cv_param.n_splits))):
            ...
            module = CoordinatedPoissonModuleArbiter(
                epochs=epochs,
    ...
    for fold_ctx, (train_data, validate_data) in ctx.on_cross_validations.ctxs_zip(kf.split(cv_data.read())):
        logger.info(f"enter fold {i}")
        if role.is_guest:
            module = CoordinatedPoissonModuleGuest(
                epochs=epochs,
                batch_size=batch_size,
                optimizer_param=optimizer,
                learning_rate_param=learning_rate_scheduler,
            ...
            module = CoordinatedPoissonModuleHost(
                epochs=epochs,
                batch_size=batch_size,
                optimizer_param=optimizer,
                learning_rate_param=learning_rate_scheduler,
...
def train_guest(ctx, train_data, validate_data, train_output_data, output_model, epochs,
                batch_size, optimizer_param, learning_rate_param, init_param, input_model, exposure_col):
    if input_model is not None:
        logger.info(f"warm start model provided")
        model = input_model.read()
        module = CoordinatedPoissonModuleGuest.from_model(model)
        module.set_epochs(epochs)
        module.set_batch_size(batch_size)
    else:
        module = CoordinatedPoissonModuleGuest(epochs=epochs, batch_size=batch_size,
                                               optimizer_param=optimizer_param, learning_rate_param=learning_rate_param,
                                               init_param=init_param, exposure_col=exposure_col)
    logger.info(f"coordinated poisson guest start train")
...
def train_host(ctx, train_data, validate_data, train_output_data, output_model, epochs, batch_size,
    ...
    if input_model is not None:
        logger.info(f"warm start model provided")
        model = input_model.read()
        module = CoordinatedPoissonModuleHost.from_model(model)
        module.set_epochs(epochs)
        module.set_batch_size(batch_size)
    else:
        module = CoordinatedPoissonModuleHost(epochs=epochs, batch_size=batch_size,
                                              optimizer_param=optimizer_param, learning_rate_param=learning_rate_param,
                                              init_param=init_param)
    logger.info(f"coordinated poisson host start train")
    sub_ctx = ctx.sub_ctx("train")
...
def train_arbiter(ctx, epochs, early_stop, tol, batch_size, optimizer_param,
    ...
    if input_model is not None:
        logger.info(f"warm start model provided")
        model = input_model.read()
        module = CoordinatedPoissonModuleArbiter.from_model(model)
        module.set_epochs(epochs)
        module.set_batch_size(batch_size)
    else:
        module = CoordinatedPoissonModuleArbiter(epochs=epochs, early_stop=early_stop, tol=tol, batch_size=batch_size,
                                                 optimizer_param=optimizer_param,
                                                 learning_rate_param=learning_rate_param,
                                                 )
    logger.info(f"coordinated poisson arbiter start train")
