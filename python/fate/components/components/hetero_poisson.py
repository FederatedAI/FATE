...
def hetero_poisson(ctx, role):
    ...


@hetero_poisson.train()
...
@cpn.parameter("exposure_col_name", type=str, default=None, desc="exposure column name")
@cpn.parameter("use_anonymous", type=bool, default=False,
               desc="bool, whether interpret `exposure_col_name` as anonymous column name")
...
def train(
    ...
    # alternatively, use len(ctx.hosts) to get the number of hosts
    if ctx.host_count > 1:
        raise ValueError("HeteroPoisson only supports single host mode")
    if role.is_guest:
        train_guest(
            ctx, train_data, validate_data, train_output_data, output_model, max_iter,
            batch_size, optimizer, learning_rate_scheduler, init_param, exposure_col_name, use_anonymous
        )
    elif role.is_host:
        train_host(
            ctx, train_data, validate_data, train_output_data, output_model, max_iter,
...
def train_guest(ctx, train_data, validate_data, train_output_data, output_model, max_iter,
                batch_size, optimizer_param, learning_rate_param, init_param, exposure_col_name, use_anonymous):
    from fate.ml.glm.hetero_poisson import HeteroPoissonModuleGuest
    # optimizer = optimizer_factory(optimizer_param)
    if use_anonymous and exposure_col_name is not None:
        columns = train_data.schema.columns.to_list()
        anonymous_columns = train_data.schema.anonymous_columns.to_list()
        exposure_col_name = columns[anonymous_columns.index(exposure_col_name)]

    with ctx.sub_ctx("train") as sub_ctx:
        module = HeteroPoissonModuleGuest(max_iter=max_iter, batch_size=batch_size,
                                          optimizer_param=optimizer_param, learning_rate_param=learning_rate_param,
                                          init_param=init_param, exposure_col_name=exposure_col_name)
        ...
        with output_model as model_writer:
            model_writer.write_model("hetero_poisson_guest", model, metadata={})
...
def train_host(ctx, train_data, validate_data, train_output_data, output_model, max_iter, batch_size,
    ...
    from fate.ml.glm.hetero_poisson import HeteroPoissonModuleHost
    # optimizer = optimizer_factory(optimizer_param)

    with ctx.sub_ctx("train") as sub_ctx:
        module = HeteroPoissonModuleHost(max_iter=max_iter, batch_size=batch_size,
                                         optimizer_param=optimizer_param, learning_rate_param=learning_rate_param,
        ...
        model = module.get_model()
        with output_model as model_writer:
            model_writer.write_model("hetero_poisson_host", model, metadata={})
    with ctx.sub_ctx("predict") as sub_ctx:
...
def train_arbiter(ctx, max_iter, early_stop, tol, batch_size, optimizer_param,
    ...
    from fate.ml.glm.hetero_poisson import HeteroPoissonModuleArbiter

    ctx.metrics.handler.register_metrics(poisson_loss=ctx.writer(train_output_metric))

    with ctx.sub_ctx("train") as sub_ctx:
        module = HeteroPoissonModuleArbiter(max_iter=max_iter, early_stop=early_stop, tol=tol, batch_size=batch_size,
                                            optimizer_param=optimizer_param, learning_rate_param=learning_rate_param)
        module.fit(sub_ctx)
...
def predict_guest(ctx, input_model, test_data, test_output_data):
    ...
    with ctx.sub_ctx("predict") as sub_ctx:
        ...
        module = HeteroPoissonModuleGuest.from_model(model)
...
def predict_host(ctx, input_model, test_data, test_output_data):
    ...
    with ctx.sub_ctx("predict") as sub_ctx:
        with input_model as model_reader:
            model = model_reader.read_model()
        module = HeteroPoissonModuleHost.from_model(model)
        test_data = sub_ctx.reader(test_data).read_dataframe()
