...
class Federation(FederationABC):
    ...
    def cleanup(self, parties):
        ...
        for party in parties:
            ...
            self._rabbit_manager.delete_connections(vhost=vhost)
    ...
    def _get_or_create_queue(self, party: Party, name=None, partitions=None, dtype=None) -> typing.Tuple: 
        queue_key_list = []
        queue_infos = []
        ...
        for queue_key in queue_key_list:  
            if queue_key not in self._queue_map:
                ...
                send_queue_name = f"send-{self._party.role}-{self._party.party_id}-{party.role}-{party.party_id}-{queue_suffix}"
                receive_queue_name = f"receive-{party.role}-{party.party_id}-{self._party.role}-{self._party.party_id}-{queue_suffix}"
                ...
                LOGGER.debug(f"[rabbitmq.get_or_create_queue]queue for queue_key: {queue_key}, party:{party} created")
    ...
    def _get_channels_index(self, index, mq_names, mq, connection_conf: dict):
        ...
        for e in mq_names:
            ...
            role = queue_key_splits[0]   
            party_id = queue_key_splits[1] 
            info = self._get_channel(mq, queue_names, party_id=party_id, role=role, connection_conf=connection_conf)
            channel_infos.append(info)
        return channel_infos
    ...
    def _receive_obj(self, channel_info, name, tag):
        ...
        for method, properties, body in channel_info.consume():
            LOGGER.debug(f"[rabbitmq._receive_obj] method: {method}, properties: {properties}.")
            if properties.message_id != name or properties.correlation_id != tag:
                # todo: fix this
                LOGGER.warning(f"[rabbitmq._receive_obj] require {name}.{tag}, got {properties.message_id}.{properties.correlation_id}")
            
            cache_key = self._get_message_cache_key(properties.message_id, properties.correlation_id, party_id, role)
            # object
            if properties.content_type == 'text/plain':
                ...
                if cache_key == wish_cache_key:
                    channel_info.cancel()
                    LOGGER.debug(f"[rabbitmq._receive_obj] cache_key: {cache_key}, obj: {self._message_cache[cache_key]}")
                    return self._message_cache[cache_key] 
            else:
                raise ValueError(f"[rabbitmq._receive_obj] properties.content_type is {properties.content_type}, but must be text/plain")

    def _send_kv(self, name, tag, data, channel_infos, partition_size, partitions, message_key):
        headers = {"partition_size": partition_size, "partitions": partitions, "message_key": message_key}
        for info in channel_infos:
            properties = pika.BasicProperties(
    ...
    def _partition_send(self, index, kvs, name, tag, partitions, mq_names, mq, maximun_message_size, connection_conf: dict):
        channel_infos = self._get_channels_index(index=index, mq_names=mq_names, mq=mq, connection_conf=connection_conf)
        ...
        for k, v in kvs:
            ...
            if datastream.get_size() + sys.getsizeof(el['k']) + sys.getsizeof(el['v']) >= maximun_message_size:
                print(f'[rabbitmq._partition_send]The size of message is: {datastream.get_size()}')
                message_key_idx += 1
    ...
    def _partition_receive(self, index, kvs, name, tag, party_id, role, party_mq_names, mq, connection_conf: dict):     
        ...
        channel_info = self._get_channel(mq, queue_names, party_id, role, connection_conf)
        ...
        for method, properties, body in channel_info.consume():
            print(f"[rabbitmq._partition_receive] method: {method}, properties: {properties}.")
            if properties.message_id != name or properties.correlation_id != tag:
                # todo: fix this
                channel_info.basic_ack(delivery_tag=method.delivery_tag)
                print(f"[rabbitmq._partition_receive]: require {name}.{tag}, got {properties.message_id}.{properties.correlation_id}")
                continue
            
            if properties.content_type == 'application/json':
                message_key = properties.headers["message_key"]
                if message_key in message_key_cache:
                    print(f"[rabbitmq._partition_receive] message_key : {message_key} is duplicated")
                    channel_info.basic_ack(delivery_tag=method.delivery_tag)
                    continue
                ...
                ValueError(f"[rabbitmq._partition_receive]properties.content_type is {properties.content_type}, but must be application/json")
