...
def upload(ctx, include, exclude, glob, suite_type, role, config_type, **kwargs):
    ...
    if len(include) != 0:
        ...
        if config_type == 'min_test':
            config_file = config.min_test_file
        else:
            config_file = config.all_examples_file
...
def download_mnists(ctx, output_path, **kwargs):
    ...
    if output_path is None:
        config = get_config(config_inst)
        output_path = os.path.join(config.data_base_dir, "/examples/data/")
    if not yes and not click.confirm("running?"):
        return
    try:
...
def query_component_output_data(clients: Clients, config: Config, component_name, job_id, role, party_id):
    ...
    try:
        ...
        try:
            ...
            table_info = clients[clients_role].table_info(table_name=table_info['name'], namespace=table_info['namespace'])
