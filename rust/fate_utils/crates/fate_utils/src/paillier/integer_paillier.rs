...
#[derive(Default)]
pub struct PK(paillier::PK);

#[pyclass(module = "fate_utils.paillier")]
#[derive(Default)]
pub struct SK(paillier::SK);

#[pyclass(module = "fate_utils.paillier")]
#[derive(Default)]
pub struct Ciphertext(paillier::CT);

#[pyclass(module = "fate_utils.paillier")]
#[derive(Default, Debug)]
pub struct CiphertextVector {
    data: Vec<paillier::CT>,
}

#[pyclass(module = "fate_utils.paillier")]
#[derive(Default, Debug)]
pub struct PlaintextVector {
    data: Vec<paillier::PT>,
}

#[pyclass(module = "fate_utils.paillier")]
#[derive(Default)]
pub struct Plaintext(paillier::PT);
...
impl PK {
    fn encrypt_encoded(
        ...
        let data = plaintext_vector.data.iter().map(|x| self.0.encrypt(&x, obfuscate)).collect();
        CiphertextVector { data }
    }
    fn encrypt_encoded_scalar(&self, plaintext: &Plaintext, obfuscate: bool) -> Ciphertext {
        Ciphertext(self.0.encrypt(&plaintext.0, obfuscate))
    }
...
impl SK {
    fn decrypt_to_encoded(&self, data: &CiphertextVector) -> PlaintextVector {
        let data = data.data.iter().map(|x| self.0.decrypt(&x)).collect();
        PlaintextVector { data }
    }
    fn decrypt_to_encoded_scalar(&self, data: &Ciphertext) -> Plaintext {
        Plaintext(self.0.decrypt(&data.0))
    }
...
impl CiphertextVector {
    ...
    pub fn zeros(size: usize) -> PyResult<Self> {
        let data = vec![paillier::CT::zero(); size];
    ...
    fn add(&self, pk: &PK, other: &CiphertextVector) -> CiphertextVector {
        let data = self.data.iter().zip(other.data.iter()).map(|(x, y)| x.add_ct(y, &pk.0)).collect();
        CiphertextVector { data }
    }
    fn add_scalar(&self, pk: &PK, other: &Ciphertext) -> CiphertextVector {
        let data = self.data.iter().map(|x| x.add_ct(&other.0, &pk.0)).collect();
        CiphertextVector { data }
    }
    fn sub(&self, pk: &PK, other: &CiphertextVector) -> CiphertextVector {
        let data = self.data.iter().zip(other.data.iter()).map(|(x, y)| x.sub_ct(y, &pk.0)).collect();
        CiphertextVector { data }
    }
    fn sub_scalar(&self, pk: &PK, other: &Ciphertext) -> CiphertextVector {
        let data = self.data.iter().map(|x| x.sub_ct(&other.0, &pk.0)).collect();
        CiphertextVector { data }
    }
    fn rsub(&self, pk: &PK, other: &CiphertextVector) -> CiphertextVector {
        let data = self.data.iter().zip(other.data.iter()).map(|(x, y)| y.sub_ct(x, &pk.0)).collect();
        CiphertextVector { data }
    }
    fn rsub_scalar(&self, pk: &PK, other: &Ciphertext) -> CiphertextVector {
        let data = self.data.iter().map(|x| other.0.sub_ct(x, &pk.0)).collect();
        CiphertextVector { data }
    }
    fn mul(&self, pk: &PK, other: &PlaintextVector) -> CiphertextVector {
        let data = self.data.iter().zip(other.data.iter()).map(|(x, y)| x.mul_pt(y, &pk.0)).collect();
        CiphertextVector { data }
    }

    fn mul_scalar(&self, pk: &PK, other: &Plaintext) -> CiphertextVector {
        let data = self.data.iter().map(|x| x.mul_pt(&other.0, &pk.0)).collect();
        CiphertextVector { data }
    }
