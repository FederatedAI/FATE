...
#[pyclass(module = "fate_utils.ou")]
#[derive(Default)]
pub struct PK(fixedpoint_ou::PK);

#[pyclass(module = "fate_utils.ou")]
#[derive(Default)]
pub struct SK(fixedpoint_ou::SK);

#[pyclass(module = "fate_utils.ou")]
#[derive(Default)]
pub struct Coder(fixedpoint_ou::Coder);

#[pyclass(module = "fate_utils.ou")]
#[derive(Default)]
pub struct Ciphertext(fixedpoint_ou::Ciphertext);

#[pyclass(module = "fate_utils.ou")]
#[derive(Default, Debug)]
pub struct CiphertextVector(fixedpoint_ou::CiphertextVector);

#[pyclass(module = "fate_utils.ou")]
#[derive(Default, Debug)]
pub struct PlaintextVector(fixedpoint_ou::PlaintextVector);

#[pyclass(module = "fate_utils.ou")]
#[derive(Default)]
pub struct Plaintext(fixedpoint_ou::Plaintext);
...
fn keygen(bit_length: u32) -> (SK, PK, Coder) {
    let (sk, pk, coder) = fixedpoint_ou::keygen(bit_length);
...
impl CiphertextVector {
    #[new]
    fn __new__() -> PyResult<Self> {
        Ok(CiphertextVector(fixedpoint_ou::CiphertextVector { data: vec![] }))
    }
    ...
    pub fn zeros(size: usize) -> PyResult<Self> {
        Ok(CiphertextVector(fixedpoint_ou::CiphertextVector::zeros(size)))
    }
...
impl PlaintextVector {
    #[new]
    fn __new__() -> PyResult<Self> {
        Ok(PlaintextVector(fixedpoint_ou::PlaintextVector { data: vec![] }))
    }
    fn __getstate__(&self) -> PyResult<Vec<u8>> {
        Ok(bincode::serialize(&self.0).unwrap())
    }
...
impl Evaluator {
    ...
    fn cat(vec_list: Vec<PyRef<CiphertextVector>>) -> PyResult<CiphertextVector> {
        let mut data = vec![fixedpoint_ou::Ciphertext::zero(); 0];
        for vec in vec_list {
            data.extend(vec.0.data.clone());
        }
        Ok(CiphertextVector(fixedpoint_ou::CiphertextVector { data }))
    }
    #[staticmethod]
