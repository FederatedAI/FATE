...
impl Coder {
    ...
    pub fn pack_floats(&self, floats: &Vec<f64>, offset_bit: usize, pack_num: usize, precision: u32) -> Vec<Plaintext> {
        ...
        floats.chunks(pack_num).map(|data| {
            ...
            Plaintext {
                significant: ou::PT(BInt(significant)),
                exp: 0,
            }
        })
            .collect()
...
pub struct Ciphertext {
    pub significant_encryped: ou::CT,
    pub exp: i32,
}
...
impl Ciphertext {
    pub fn zero() -> Ciphertext {
        Ciphertext {
            significant_encryped: ou::CT::zero(),
            exp: 0,
        }
    }
    fn decrese_exp_to(&self, exp: i32, pk: &ou::PK) -> Ciphertext {
        assert!(exp < self.exp);
        let factor = BInt::from(BASE).pow((self.exp - exp) as u32);
        let significant_encryped = self.significant_encryped.mul_pt(&ou::PT(factor), pk);
        Ciphertext {
    ...
    pub fn neg(&self, pk: &PK) -> Ciphertext {
        Ciphertext {
            significant_encryped: ou::CT(self.significant_encryped.0.invert_ref(&pk.pk.n)),
            exp: self.exp,
        }
    }
    ...
    */
    ...
    pub fn i_double(&mut self, pk: &PK) {
        self.significant_encryped.0 = self
            .significant_encryped
            .0
            .pow_mod_ref(&BInt::from(2), &pk.pk.n);
    }
...
impl CiphertextVector {
    ...
    pub fn pack_squeeze(&self, pk: &PK, pack_num: usize, shift_bit: u32) -> CiphertextVector {
        ...
        let data = self.data.chunks(pack_num).map(|x| {
            ...
            for y in &x[1..] {
                result.pow_mod_mut(&base, &pk.pk.n);
                result = result.mul(&y.significant_encryped.0) % &pk.pk.n;
            }
            Ciphertext { significant_encryped: ou::CT(result), exp: 0 }
        }).collect();
        CiphertextVector { data }
    }
...
#[test]
fn test_decrypt() {
    let (sk, pk, coder) = keygen(1024);
    let mut data = vec![0.5, -0.5];
    let encoded = PlaintextVector { data: data.iter().map(|x| coder.encode_f64(*x)).collect() };
    let encrypted = pk.encrypt_encoded(&encoded, false);
    let decrypted = sk.decrypt_to_encoded(&encrypted);
    let decoded = decrypted.data.iter().map(|x| coder.decode_f64(x)).collect::<Vec<_>>();
