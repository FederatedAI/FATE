...
buildModule() {
  ...
  do
      ...
      docker build --build-arg PREFIX=${PREFIX} --build-arg BASE_TAG=${BASE_TAG} -t ${PREFIX}/${module}:${TAG} -f ${WORKINGDIR}/docker/modules/${module}/Dockerfile ./cluster-deploy/example-dir-tree/${module}/
