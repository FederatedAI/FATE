...
class TestTable(unittest.TestCase):
    ...
    def test_table_info(self):
        response = requests.post("/".join([self.server_url, 'table', 'table_info']), json={'table_name': 'breast_b', 'namespace': 'fate_flow_tabletest_breast'})
        self.assertTrue(response.status_code in [200, 201])
        self.assertTrue(int(response.json()['retcode']) == 0)

    def test_table_delete(self):
        # submit
        response = requests.post("/".join([self.server_url, 'table', 'delete']), json={'table_name': 'breast_b', 'namespace': 'fate_flow_tabletest_breast'})
        self.assertTrue(response.status_code in [200, 201])
        self.assertTrue(int(response.json()['retcode']) == 0)
