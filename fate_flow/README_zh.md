...
### server.conf
在 server.conf 配置所有 FATE 服务的地址. 不同部署模式的 FATE-Flow 需要不同的Fate服务. 有关详细信息, 请参考以下特定的部署模式.
...
```
请使用您的作业(job) 配置(job configuration) 替换publish_online_model.json中的相应配置。
之后, FATE-Serving会使用您提供的配置来设置参与方的默认模型ID和涉及该模型ID的默认模型版本。
然后, 您可以仅通过指定party_id或模型ID来向FATE-Serving提出在线推理请求。
...
- FATE-Flow是一个调度系统, 可根据用户提交的作业(job) 的DSL调度算法组件的执行。
