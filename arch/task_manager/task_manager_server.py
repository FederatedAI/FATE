...
from arch.task_manager.job_manager import push_into_job_queue, pop_from_job_queue, \
    get_job_from_queue, running_job_amount, update_job_queue, generate_job_id, get_job_directory, \
    save_job_info, is_job_initiator, query_job_by_id, check_job_process
                ...
                if wait_jobs:
                    ...
                    if run_job_success:
                        update_job_queue(job_id=run_job_id,
                                         my_role=wait_job.my_role,
                                         my_party_id=wait_job.my_party_id,
                                         **{"status": "ready"})
                    else:
...
def stop_job(job_id):
    ...
    for runtime_conf_path in glob.glob(os.path.join(_job_dir, '**', 'runtime_conf.json'), recursive=True):
        ...
        for _role, _party_ids in runtime_conf['role'].items():
            _roles.extend([(_role, _party_id) for _party_id in _party_ids])
    _roles = set(_roles)
    logger.info('start send stop job to {}'.format(','.join([i[0] for i in _roles])))
    _method = 'DELETE'
    for _role, _party_id in _roles:
