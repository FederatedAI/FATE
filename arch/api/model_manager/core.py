...
from arch.api.io.version_control import version_for_read, version_for_save, gen_data_table_info, get_data_table, save_version_info, version_history


def save_model(buffer_type, proto_buffer, commit_log="", branch="master"):
    version_table, data_table_info, scene_key, parent, commit_id = version_for_save("model_version", branch=branch)
...
def read_model(buffer_type, proto_buffer, commit_id=None, tag=None, branch="master"):
    version_table, data_table_info, scene_key, parent, commit_id = version_for_read("model_version",
                                                                                    commit_id=commit_id,
                                                                                    tag=tag,
                                                                                    branch=branch)
    if commit_id:
