...
class FederationRuntime(object):
    ...
    def _get_all(self, name, tag):
        ...
        if len(rtn) > 0 and isinstance(rtn[0], DTable):
            rtn = [RDDTable.from_dtable(job_id=self._job_id, dtable=tbl) for tbl in rtn]
        return rtn

    def _get_single(self, name, tag, idx):
        rtn = self._eggroll_federation.get(name=name, tag=tag, idx=idx)
        if isinstance(rtn, DTable):
            rtn = RDDTable.from_dtable(job_id=self._job_id, dtable=rtn)
        return rtn
