...
class Builder1x(build1x.Builder):
    _table_cls = RDDTable
    ...
    def build_session(self):
        from arch.api.impl.based_1x.session import build_eggroll_session, build_eggroll_runtime
        from arch.api.impl.based_spark.session import build_session, broadcast_eggroll_session
        eggroll_session = build_eggroll_session(work_mode=self._work_modemode, job_id=self._session_id)
        ...
        eggroll_runtime = build_eggroll_runtime(work_mode=self._work_mode, eggroll_session=eggroll_session)
        return build_session(self._session_id, eggroll_runtime, self._persistent_engine)


class Builder2x(build2x.Builder):
    _table_cls = RDDTable
