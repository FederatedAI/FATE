...
@Component
@Scope("prototype")
public class MapServiceProcessor extends BaseProcessServiceProcessor<Processor.UnaryProcess, StorageBasic.StorageLocator> {
    public MapServiceProcessor(EggProcessServiceClient eggProcessServiceClient, Processor.UnaryProcess request, BasicMeta.Endpoint processorEndpoint) {
        super(eggProcessServiceClient, request, processorEndpoint);
    }

    @Override
    public StorageBasic.StorageLocator call() throws Exception {
        return eggProcessServiceClient.map(request, processorEndpoint);
    }
}
