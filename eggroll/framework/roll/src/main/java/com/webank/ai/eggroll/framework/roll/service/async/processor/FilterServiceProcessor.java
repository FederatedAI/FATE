...
@Component
@Scope("prototype")
public class FilterServiceProcessor extends BaseProcessServiceProcessor<Processor.UnaryProcess, StorageBasic.StorageLocator> {
    public FilterServiceProcessor(EggProcessServiceClient eggProcessServiceClient, Processor.UnaryProcess request, BasicMeta.Endpoint processorEndpoint) {
        super(eggProcessServiceClient, request, processorEndpoint);
    }

    @Override
    public StorageBasic.StorageLocator call() throws Exception {
        return eggProcessServiceClient.filter(request, processorEndpoint);
    }
}
