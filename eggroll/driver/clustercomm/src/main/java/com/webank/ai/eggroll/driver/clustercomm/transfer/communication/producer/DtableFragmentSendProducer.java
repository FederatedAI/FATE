...
public class DtableFragmentSendProducer extends BaseProducer {
    ...
    public BasicMeta.ReturnStatus call() throws Exception {
        ...
        LOGGER.info("[CLUSTERCOMM][PRODUCER][DTABLE] dtable send producer: {}", fragmentString);
        ...
        try {
            ...
            try (KeyValueIterator<Bytes, byte[]> iterator = keyValueStore.all()) {
                LOGGER.info("[CLUSTERCOMM][PRODUCER][DTABLE] count of keyValueStore {}: {}", storageLocatorString, keyValueStore.count());
                ...
                if (entryCount == 0) {
                    LOGGER.warn("[CLUSTERCOMM][PRODUCER][DTABLE] empty entryCount: {}", fragmentString);
                }
                // transferBroker.setFinished();
                result = returnStatusFactory.createSucessful("[CLUSTERCOMM][PRODUCER][DTABLE] fragment sendProducer Done. fragment: " + fragmentString);
            } // end try resource
        } catch (Exception e) {
            LOGGER.info("[CLUSTERCOMM][PRODUCER][DTABLE][ERROR] Fragment send producer exception: " + errorUtils.getStackTrace(e));
            throw e;
        } finally {
            LOGGER.info("[CLUSTERCOMM][PRODUCER][DTABLE] finish producing data for {}, total entryCount: {}, packetCount: {}", storageLocatorString, entryCount, packetCount);
