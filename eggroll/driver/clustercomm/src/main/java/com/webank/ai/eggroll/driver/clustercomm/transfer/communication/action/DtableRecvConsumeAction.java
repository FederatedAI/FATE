...
import com.webank.ai.eggroll.driver.clustercomm.transfer.utils.PrintUtils;
...
public class DtableRecvConsumeAction extends BaseRecvConsumeAction {
    ...
    public void onInit() {
        ...
        if (!transferMeta.hasDataDesc() || !transferMeta.getDataDesc().hasStorageLocator()) {
            ...
            ClusterComm.TransferDataDesc passedInTransferDataDesc = passedInTransferMeta.getDataDesc();
            ...
            ClusterComm.TransferDataDesc.Builder dataDescBuilder = ClusterComm.TransferDataDesc.newBuilder();
        ...
        ClusterComm.TransferDataDesc finalTransferDataDesc = finalTransferMeta.getDataDesc();
        ...
        putallListenableFuture.addCallback(new ListenableFutureCallback<Object>() {
            ...
            public void onFailure(Throwable throwable) {
                LOGGER.error("[CLUSTERCOMM][RECV][CONSUMER][DTABLE] addAll failed for data: {}, error: {}",
            ...
            public void onSuccess(Object o) {
                LOGGER.info("[CLUSTERCOMM][RECV][CONSUMER][DTABLE] addAll finished for data: {}",
                        transferMetaId);
                putAllFinishLatch.countDown();
            }
        });
    ...
    public void onComplete() {
        LOGGER.info("[CLUSTERCOMM][SEND][DTABLE][CONSUMER] trying to complete DtableRecvConsumeAction. entryCount: {}, transferMetaId: {}",
                entryCount, transferMetaId);
        while (!transferBroker.isClosable()) {
            onProcess();
        }

        LOGGER.info("[CLUSTERCOMM][SEND][DTABLE][CONSUMER] actual completes DtableRecvConsumeAction. entryCount: {}, transferMetaId: {}",
                entryCount, transferMetaId);
        ...
        try {
            ...
            while (!latchAwaitResult) {
                ...
                if (((now - startWaitTime) / 1000) % 10 == 0) {
                    LOGGER.info("[CLUSTERCOMM][RECV][CONSUMER][DTABLE] addAll waiting to finished: {}", transferMetaId);
                }

                // todo: add timeout mechanism
            }
        } catch (Exception e) {
            LOGGER.error("[CLUSTERCOMM][RECV][CONSUMER][DTABLE] addAll latch await failed. exception: {}", errorUtils.getStackTrace(e));
        }
