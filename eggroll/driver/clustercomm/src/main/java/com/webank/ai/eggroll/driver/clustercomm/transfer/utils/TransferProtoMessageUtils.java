...
package com.webank.ai.eggroll.driver.clustercomm.transfer.utils;
...
public class TransferProtoMessageUtils {
    ...
    public Proxy.Topic partyToTopic(ClusterComm.Party party) {
        if (!inited) {
            init();
        }
    ...
    public ClusterComm.TransferMeta extractTransferMetaFromPacket(Proxy.Packet packet) {
        ByteString serialized = packet.getBody().getValue();

        Preconditions.checkNotNull(serialized, "null serialized transferMeta");
        try {
            return ClusterComm.TransferMeta.parseFrom(serialized);
        } catch (InvalidProtocolBufferException e) {
            throw new SerializationException(e);
        }
    }

    public Proxy.Packet generateSendStartRequest(ClusterComm.TransferMeta transferMeta) {
        return generateSendCommandRequest(transferMeta, COMMAND_SEND_START);
    }

    public Proxy.Packet generateSendEndRequest(ClusterComm.TransferMeta transferMeta) {
        return generateSendCommandRequest(transferMeta, COMMAND_SEND_END);
    }

    private Proxy.Packet generateSendCommandRequest(ClusterComm.TransferMeta transferMeta, Proxy.Command command) {
        if (!inited) {
            init();
        }
